...
@pytest.mark.parametrize(
    ("mutate", "expected"),
    [
        (lambda study: study.update(classification="interface_fixture"), FIXTURE_NOT_ESTIMABLE),
        (lambda study: study.update(assumptions=[]), MISSING_IDENTIFICATION),
        (lambda study: study["question"].pop("treatment"), MISSING_TREATMENT_DEFINITION),
        (lambda study: study["question"].pop("outcome"), MISSING_OUTCOME_DEFINITION),
        (lambda study: study["sources"].clear(), MISSING_DATA_COVERAGE),
        (lambda study: study["diagnostics"].__setitem__(0, {"name": "balance", "status": "not_run", "evidence": "recorded"}), MISSING_DIAGNOSTICS),
    ],
)
...
def test_validator_returns_all_relevant_failed_criteria_without_data_values() -> None:
    ...
    assert all("source-1" not in diagnostic.message for diagnostic in result.diagnostics)


def test_minimally_valid_artifact_requires_explicit_method_and_passing_diagnostics() -> None:
    study = _study()
