...
from scripts.validate_commercial_buildings_kit import (
    CATALOG_PATH,
    KIT_DIR,
    META_PATH,
    SOURCE_PATH,
...
def test_commercial_buildings_kit_matches_the_shared_contract():
    assert validate_kit() == []


def test_commercial_buildings_kit_has_no_minnesota_identity_claim():
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            "archetype_id must be commercial_buildings",
        ),
        (
        ...
        (
            lambda meta: meta["bounds_m"].update(max=[49.0, 999.0, 54.0]),
...
def test_catalog_drift_turns_the_kit_red(tmp_path):
    ...
    archetype = next(
        item for item in catalog["archetypes"] if item["id"] == "commercial_buildings"
    )
