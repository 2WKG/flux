...
ASSET_DIR = (
    Path(__file__).resolve().parents[1] / "data/3d/assets/factory_industrial_facility"
)


def test_factory_asset_matches_shared_contract():
    root = Path(__file__).resolve().parents[1]
    catalog = json.loads((root / "data/3d/asset-archetypes-v1.json").read_text())
    meta = json.loads((ASSET_DIR / "factory_industrial_facility.meta.json").read_text())
    archetype = next(
