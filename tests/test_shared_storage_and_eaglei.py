...
def test_shared_store_replaces_empty_slice_and_exports_parquet(tmp_path) -> None:
    ...
    try:
        ...
        assert (
            con.execute(
                "SELECT count(*) FROM read_parquet(?)", [str(target / "buses.parquet")]
            ).fetchone()[0]
            == 0
        )
    finally:
...
def test_retrieval_timestamp_is_preserved_or_honestly_unknown(tmp_path) -> None:
    ...
    try:
        ...
        replace_frame(
            ...
            fixture_batch_id="known",
        )
        assert con.execute("SELECT source_retrieved_at FROM counties").fetchone()[
            0
        ] == retrieved.replace(tzinfo=None)
        target = tmp_path / "parquet"
        export_parquet(con, target)
        assert con.execute(
            "SELECT source_retrieved_at FROM read_parquet(?)",
            [str(target / "counties.parquet")],
        ).fetchone()[0] == retrieved.replace(tzinfo=None)
        replace_frame(
            con,
            "counties",
            seed,
            source_name="unknown",
            source_ref="no-receipt",
            fixture_batch_id="unknown",
        )
        assert (
            con.execute("SELECT source_retrieved_at FROM counties").fetchone()[0]
            is None
        )
    finally:
        con.close()
...
def test_eaglei_replay_uses_source_year_not_utc_year(tmp_path) -> None:
        ...
        [
            {
                ...
                "state": "Texas",
                "customers_out": 7,
                "run_start_time": "2021-12-31 20:00:00",
                "total_customers": 10,
            }
        ],
    )
    ...
    try:
        ...
        assert (
            con.execute("SELECT EXTRACT(year FROM ts) FROM eaglei_outages").fetchone()[
                0
            ]
            == 2022
        )
        ...
        assert (
            con.execute("SELECT count(*) FROM eaglei_outage_observations").fetchone()[0]
            == 0
        )
    finally:
        con.close()
...
def test_legacy_and_minnesota_namespaces_coexist(tmp_path) -> None:
    ...
    try:
        ...
        assert con.execute(
            "SELECT contract_version FROM mn_schema_meta"
        ).fetchone() == ("2.0.0-mn",)
    finally:
        con.close()
...
def test_storm_replay_uses_source_year_not_converted_timestamp_year(tmp_path) -> None:
    ...
    pd.DataFrame(
        [
            {
                "EVENT_ID": 1,
                "BEGIN_DATE_TIME": "31-DEC-21 20:00:00",
                "END_DATE_TIME": "31-DEC-21 21:00:00",
    ...
    try:
        ...
        assert (
            con.execute(
                "SELECT EXTRACT(year FROM ts_begin) FROM storm_events"
            ).fetchone()[0]
            == 2022
        )
        pd.DataFrame(
...
def test_parent_refresh_preserves_foreign_keys_and_removes_absent_unreferenced_rows(
    tmp_path,
):
    con = connect(tmp_path / "grid.duckdb")
    provenance = {
    ...
    try:
        ...
        replace_frame(
            con,
            "hazard_static",
            pd.DataFrame([{"county_fips": "48001", "nri_score": 2}]),
            **provenance,
        )
        buses = pd.DataFrame(
            [
        ...
        replace_frame(
            con,
            "loads",
            pd.DataFrame([{"load_id": 1, "bus_id": 1, "p_mw_nominal": 5}]),
            **provenance,
        )
        counties = counties.iloc[:1].copy()
        counties["pop"] = 10
        replace_frame(
            con, "counties", counties, **{**provenance, "fixture_batch_id": "v2"}
        )
        buses["name"] = "updated bus"
        replace_frame(con, "buses", buses, **provenance)
        assert con.execute(
            "SELECT county_fips, pop, fixture_batch_id FROM counties"
        ).fetchall() == [("48001", 10, "v2")]
