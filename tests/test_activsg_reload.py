...
def test_topology_links_counties_before_children_and_reloads(tmp_path, monkeypatch):
    ...
    buses[:, 2] = 1
    buses[:, 9] = 230
    branch = np.array([[1, 2, .01, .1, 0, 100, 0, 0, 0, 0, 1]])
    ...
    monkeypatch.setattr(activsg, "_string_cell", lambda _text, name:
                        [str(i) for i in range(2000)] if name == "bus_name" else ["gas"])
    coords = pd.DataFrame({"bus_id": np.arange(1, 2001), "base_kv_aux": 230,
                           "lon": -95., "lat": 30., "sub_num": np.arange(1, 2001),
                           "sub_name": "fixture", "sub_id": np.arange(1, 2001)})
    ...
    try:
        county = pd.DataFrame([{"county_fips": "48001", "name": "fixture", "state": "TX",
                                "pop": 1, "geom_wkb": box(-96, 29, -94, 31).wkb}])
        for _ in range(2):
            ...
            assert con.execute("SELECT count(*) FROM buses WHERE county_fips = '48001'").fetchone() == (2000,)
