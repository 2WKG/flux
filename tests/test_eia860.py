...
def _write_eia_parquet(
    tmp_path, plants: list[dict], generators: list[dict]
) -> tuple[str, str]:
...
def test_eia_plants_uses_latest_report_per_generator_and_keeps_inventory_history(
    tmp_path,
):
                ...
                [
                    {
                        ...
                        "pop": 1,
                        "geom_wkb": Polygon(
                            [(-110, 25), (-90, 25), (-90, 40), (-110, 40)]
                        ).wkb,
                    }
                ]
            ),
            source_name="test",
            source_ref="eia-fixture",
            fixture_batch_id="test",
        )
        load_eia860_plants(con, plants_path, generators_path)
        assert (
            con.execute("SELECT count(*) FROM eia_generator_inventory").fetchone()[0]
            == 3
        )
        assert (
            con.execute(
                "SELECT capacity_mw FROM eia_plants WHERE plant_id_eia = 1"
            ).fetchone()[0]
            == 170
        )
    finally:
...
def test_site_candidates_exclude_active_coal_and_classify_retired_and_retiring_sites(
    tmp_path,
):
    plants = [
        {
            ...
            "report_date": "2025-01-01",
            "latitude": 30.0 + plant_id,
            "longitude": -97.0 - plant_id,
            "state": "TX",
            "plant_name_eia": name,
        }
    ...
    generators = [
        ...
        },
        {
            ...
            "fuel_type_code_pudl": "coal",
            "operational_status": "retired",
            "generator_retirement_date": "2021-06-01",
        },
        {
            "plant_id_eia": 2,
            "generator_id": "B",
    ...
    try:
        ...
        assert con.execute(
            "SELECT name, kind FROM site_candidates ORDER BY name"
        ).fetchall() == [
