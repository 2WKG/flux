...
def _published_state(db_path: Path, parquet_dir: Path) -> tuple[tuple[str, ...], str, dict[str, bytes]]:
    con = connect(db_path, read_only=True)
    try:
        namespaces = tuple(row[0] for row in con.execute("SELECT value FROM mn_private ORDER BY value").fetchall())
        marker = con.execute("SELECT value FROM release_marker").fetchone()[0]
    finally:
        con.close()
    files = {path.name: path.read_bytes() for path in parquet_dir.iterdir() if path.is_file()}
    return namespaces, marker, files


def _staged_builder(marker: str):
    def build_stage(_raw_dir: str, db_path: str, _tz: str | None, parquet_dir: str) -> dict[str, int]:
        con = connect(db_path)
        try:
...
def test_preflight_failure_does_not_create_or_change_live_release(tmp_path, monkeypatch):
...
def test_loader_failure_never_publishes_staged_database_or_parquet(tmp_path, monkeypatch, phase):
    monkeypatch.chdir(tmp_path)
    db_path, parquet_dir = _seed_live_release(tmp_path)
    before = _published_state(db_path, parquet_dir)
    monkeypatch.setattr(build_module, "_missing_p0_inputs", lambda *_args: [])

    def fail_after_staging(_raw_dir: str, stage_db: str, _tz: str | None, stage_parquet: str) -> dict[str, int]:
...
def test_successful_publish_preserves_existing_namespaces_and_parquet(tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    db_path, parquet_dir = _seed_live_release(tmp_path)
    monkeypatch.setattr(build_module, "_missing_p0_inputs", lambda *_args: [])
    monkeypatch.setattr(build_module, "_build_mutating", _staged_builder("new"))
    ...
    assert build_module.build(str(tmp_path / "raw"), str(db_path), "UTC") == {"staged": 1}
