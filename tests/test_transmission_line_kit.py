...
from scripts.validate_transmission_line_kit import (
    CATALOG_PATH,
    KIT_DIR,
    META_PATH,
    SOURCE_PATH,
...
def test_transmission_line_kit_matches_the_shared_contract():
    assert validate_kit() == []


def test_transmission_line_kit_has_no_minnesota_identity_claim():
...
def test_authoring_frame_round_trips_through_the_gltf_export_mapping():
    ...
    """
    for point in ((0.0, 18.0, 6.0), (-4.5, 10.0, -4.5), (1.0, 2.0, 3.0)):
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            "archetype_id must be transmission_line_segment",
        ),
        (
        ...
        (
            lambda meta: meta["connectors"][0].update(name="CONN_HV_SIDE_0"),
            "metadata must expose the two named HV connectors",
        ),
        (
            lambda meta: meta["connectors"][0].update(role="MV_FEED"),
        ...
        (
            lambda meta: meta["bounds_m"].update(max=[5.5, 999.0, 6.0]),
...
def test_catalog_drift_turns_the_kit_red(tmp_path):
    ...
    archetype = next(
        ...
        if item["id"] == "transmission_line_segment"
    )
