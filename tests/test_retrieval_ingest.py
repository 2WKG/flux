...
def test_ingest_is_idempotent_and_preserves_project_document_provenance() -> None:
    ...
    try:
        ...
        assert (
            chunk.provenance["license_or_terms"] == "Project repository documentation"
        )
...
def test_ingest_rejects_page_less_or_mixed_evidence_instead_of_inventing_citations() -> (
    None
):
    con = duckdb.connect(":memory:")
    try:
        page_less = SourceDocument(
            document_id="page-less",
            version="v1",
            source_uri="file.md",
            text="Evidence.",
            content_kind="source",
