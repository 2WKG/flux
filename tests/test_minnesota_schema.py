...
def test_initialization_is_idempotent_and_preserves_rows():
    ...
    con.execute("INSERT INTO mn_artifact_manifests VALUES ('mn:fixture:0000000000000000','fixture',?,'mn','unavailable','not_applicable','{}',CURRENT_TIMESTAMP,'[]','[\"not built\"]','[]')", [SCHEMA_VERSION])
    ensure_minnesota_schema(con)
...
def test_incompatible_version_fails_before_mutation():
    con = duckdb.connect(":memory:")
    con.execute("CREATE TABLE mn_schema_meta (key TEXT PRIMARY KEY, value TEXT NOT NULL)")
...
def test_coordinates_and_available_provenance_are_enforced():
    con = duckdb.connect(":memory:")
    ensure_minnesota_schema(con)
    con.execute("INSERT INTO mn_artifact_manifests VALUES ('mn:geography:0000000000000000','geography',?,'mn','available','not_applicable','{}',CURRENT_TIMESTAMP,'[]','[\"limit\"]','[]')", [SCHEMA_VERSION])
...
def test_legacy_schema_and_rows_remain_untouched():
    ...
    assert con.execute("SELECT value FROM schema_meta WHERE key='legacy_marker'").fetchone() == ("kept",)
    assert con.execute("SELECT value FROM mn_schema_meta WHERE key='contract_version'").fetchone() == (SCHEMA_VERSION,)


def test_malformed_coordinates_and_provenance_are_rejected_by_database():
    con = duckdb.connect(":memory:")
    ensure_minnesota_schema(con)
    con.execute("INSERT INTO mn_artifact_manifests VALUES ('mn:g:1','geography',?,'mn','unavailable','not_applicable','{}',CURRENT_TIMESTAMP,'[]','[\"limit\"]','[]')", [SCHEMA_VERSION])
