...
def test_streams_eaglei_and_selects_latest_actual_denominator(
        ...
        [
            {
                ...
                "state": "Texas",
                "customers_out": 7,
                "run_start_time": "2021-12-31 19:00:00",
                "total_customers": 10,
            },
            {
                "fips_code": "48001",
    ...
    try:
        ...
        monkeypatch.setattr(
            eaglei.pd,
            "read_csv",
            lambda *_args, **_kwargs: pytest.fail("annual CSV hit pandas"),
        )
        ...
        assert con.execute(
            "SELECT EXTRACT(year FROM ts) FROM eaglei_outages ORDER BY ts DESC LIMIT 1"
        ).fetchone() == (2022,)
        assert eaglei.load_eaglei(con, str(source), 2021, "America/Chicago") == 2
        assert con.execute("SELECT count(*) FROM eaglei_outages").fetchone() == (2,)
        assert con.execute(
            "SELECT count(*) FROM county_customers WHERE source = 'eaglei_file'"
        ).fetchone() == (1,)
    finally:
        con.close()


def test_empty_source_year_removes_prior_slice_without_pandas(
    tmp_path, monkeypatch
) -> None:
    source = tmp_path / "eaglei_outages_2021.csv"
    _write(
        ...
        [
            {
                ...
                "state": "Texas",
                "customers_out": 7,
                "run_start_time": "2021-02-16 19:00:00",
                "total_customers": 10,
            }
        ],
    )
...
def test_duplicate_county_timestamp_rejects_before_replacing_slice(tmp_path) -> None:
    ...
    try:
        ...
        with pytest.raises(ValueError, match="duplicate"):
            eaglei.load_eaglei(con, str(source), 2021, "America/Chicago")
        assert con.execute("SELECT customers_out FROM eaglei_outages").fetchone() == (
            7,
        )
    finally:
        con.close()
