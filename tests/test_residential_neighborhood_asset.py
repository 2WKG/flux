...
ASSET_DIR = (
    Path(__file__).resolve().parents[1] / "data/3d/assets/residential_neighborhood"
)
...
def test_residential_asset_source_matches_the_shared_archetype_contract():
    catalog = json.loads(
        (
    ...
    archetype = next(
        ...
        if item["id"] == "residential_neighborhood"
    )
    meta = _read("residential_neighborhood.meta.json")
...
def test_residential_asset_source_has_neutral_status_slot_and_feeder_connector():
    meta = _read("residential_neighborhood.meta.json")
    scene = _read("residential_neighborhood.scene.json")
