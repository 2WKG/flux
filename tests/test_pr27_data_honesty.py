...
def test_bus_county_ties_choose_lowest_county_fips_independent_of_county_order(
    tmp_path,
):
    ...
    try:
        ...
        assert con.execute(
            "SELECT county_fips FROM buses WHERE bus_id = 1"
        ).fetchone() == ("48001",)
    finally:
        con.close()


def test_eia_candidates_are_source_stable_and_only_attach_to_same_county_high_voltage_bus(
    tmp_path,
):
    plants = pd.DataFrame(
        [
    ...
    plants_path, generators_path = (
        tmp_path / "plants.parquet",
        tmp_path / "generators.parquet",
    )
    ...
    try:
        ...
        first = con.execute(
            "SELECT site_id, kind, bus_id, capacity_slot_mw, source_site_id FROM site_candidates"
        ).fetchall()
...
def test_dod_ids_and_geometry_replacement_are_stable_and_do_not_delete_other_sources(
    tmp_path,
):
    ...
    try:
        ...
        con.execute(
            "CREATE TABLE critical_load_geometry(cl_id INTEGER PRIMARY KEY, source_id TEXT, reporting_component TEXT, operational_status TEXT, is_joint_base BOOLEAN, area_km2 DOUBLE, geom_wkb BLOB)"
        )
        con.execute(
            "INSERT INTO critical_load_geometry VALUES (999, 'hospital:stable', NULL, NULL, NULL, NULL, NULL)"
        )
        ...
        assert con.execute(
            "SELECT source_id FROM critical_load_geometry WHERE cl_id = 999"
        ).fetchone() == ("hospital:stable",)
        assert con.execute(
            "SELECT source_id FROM critical_load_geometry WHERE cl_id = ?", [first]
        ).fetchone() == ("mirtaLocationsIdpk:77",)
    finally:
        con.close()
