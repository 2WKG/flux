...
def test_incomplete_p0_build_does_not_mutate_or_export(tmp_path, monkeypatch) -> None:
    ...
    with pytest.raises(
        build_module.IncompleteP0BuildError, match="EIA930_BALANCE_2024_Jul_Dec"
    ):
...
def test_p0_preflight_reads_expected_paths_from_shared_dataset_catalog(
        ...
        json.dumps(
            {
                "p0_raw_inputs": [
                    {
                        "label": "fixture input",
                        "paths": [["current", "input.csv"], ["legacy.csv"]],
                    }
                ]
            }
        )
