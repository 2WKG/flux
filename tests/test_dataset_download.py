...
def test_download_retries_a_truncated_response_without_promoting_it(
    tmp_path, monkeypatch
) -> None:
    responses = iter([_Response(b"short", 7), _Response(b"complete", 8)])
    monkeypatch.setattr(
        download.urllib.request, "urlopen", lambda *_args, **_kwargs: next(responses)
    )
    monkeypatch.setattr(download.time, "sleep", lambda *_args: None)
    destination = tmp_path / "artifact.csv"

    download.download_file(
        "https://example.test/artifact.csv", destination, force=False, attempts=2
    )
...
def test_download_rejects_checksum_mismatch_without_promoting(
    tmp_path, monkeypatch
) -> None:
    monkeypatch.setattr(
        download.urllib.request,
        "urlopen",
        lambda *_args, **_kwargs: _Response(b"complete", 8),
    )
    destination = tmp_path / "artifact.csv"

    with pytest.raises(RuntimeError, match="checksum"):
        download.download_file(
            "https://example.test/artifact.csv",
            destination,
            force=False,
