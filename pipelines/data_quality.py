...
ACCEPTED_VALUES = {
    ...
    ("line_upgrade_detail", "congestion_method"): {
        "exact",
        "fuzzy",
        "twin_proxy",
        "unmapped",
...
def _database_checks(
    ...
    try:
        ...
        if version != (SCHEMA_VERSION,):
            raise RuntimeError(
                f"schema_meta contract_version is {version!r}, expected {(SCHEMA_VERSION,)!r}."
            )
        ...
        if missing_constraints:
            raise RuntimeError(
                f"schema is missing {len(missing_constraints)} v1 constraint(s)."
            )
        ...
        alerts.append(
            _alert(
                "schema_contract",
                str(exc),
                next_step="Rebuild or migrate the artifact before publishing it.",
            )
        )
        ...
        if required:
            ...
            if missing:
                alerts.append(
                    _alert(
                        "null_required",
                        f"{table} has {missing} row(s) with a required value missing.",
                        next_step="Repair the loader output and rebuild the curated artifact.",
                    )
                )
        primary_key = [
        ...
        if primary_key:
            ...
            if duplicates:
                alerts.append(
                    _alert(
                        "duplicate_key",
                        f"{table} has {duplicates} duplicate primary-key group(s).",
                        next_step="Deduplicate the source release and rerun the loader.",
                    )
                )
        ...
        if missing:
            alerts.append(
                _alert(
                    ...
                    f"{child}.{child_col} has {missing} orphaned reference(s) to {parent}.{parent_col}.",
                    next_step="Load the parent artifact or correct the foreign key before promotion.",
                )
            )
    ...
    for (table, column), values in ACCEPTED_VALUES.items():
        ...
        invalid = con.execute(
            f"SELECT count(*) FROM {table} WHERE {column} IS NOT NULL AND {column} NOT IN ({allowed})",
            list(values),
        ).fetchone()[0]
        if invalid:
            alerts.append(
                _alert(
                    "accepted_values",
                    f"{table}.{column} has {invalid} value(s) outside its contract.",
                    next_step="Map source values to the contract enum or reject the release.",
                )
            )
    return counts, alerts
...
def _curated_source_mappings(
    operations: dict[str, Any],
) -> dict[tuple[str, str | None], SourceMapping]:
    ...
    known_ids = {
        str(source["id"])
        for source in operations.get("sources", [])
        if source.get("id")
    }
    ...
    for index, mapping in enumerate(operations.get("curated_source_mappings", [])):
        ...
        if not isinstance(source_name, str) or not source_name:
            raise ValueError(
                f"curated_source_mappings[{index}] requires a nonempty source_name"
            )
        if source_version is not None and (
            not isinstance(source_version, str) or not source_version
        ):
            raise ValueError(
                f"curated_source_mappings[{index}].source_version must be a nonempty string when set"
            )
        if (
            ...
            raise ValueError(
                f"curated_source_mappings[{index}] requires nonempty operation_ids"
            )
        if len(set(operation_ids)) != len(operation_ids):
            raise ValueError(
                f"curated_source_mappings[{index}] has duplicate operation_ids"
            )
        reconciliation_ids = mapping.get("reconciliation_operation_ids", operation_ids)
        if (
            not isinstance(reconciliation_ids, list)
            or not reconciliation_ids
            or any(not isinstance(item, str) or not item for item in reconciliation_ids)
        ):
            raise ValueError(
                f"curated_source_mappings[{index}] requires nonempty reconciliation_operation_ids when set"
            )
        if len(set(reconciliation_ids)) != len(reconciliation_ids):
            raise ValueError(
                f"curated_source_mappings[{index}] has duplicate reconciliation_operation_ids"
            )
        if not set(reconciliation_ids) <= set(operation_ids):
            raise ValueError(
                f"curated_source_mappings[{index}] reconciliation_operation_ids must be declared operation_ids"
            )
        unknown_ids = set(operation_ids) - known_ids
        if unknown_ids:
            raise ValueError(
                f"curated_source_mappings[{index}] references unknown operations ID(s): {sorted(unknown_ids)}"
            )
        key = (source_name, source_version)
        if key in mappings:
            raise ValueError(
                f"curated_source_mappings has duplicate mapping for {source_name!r}, {source_version!r}"
            )
...
def _operation_ids_for_curated_source(
    ...
    *,
    ...
    return mappings.get(
        (source_name, source_version), mappings.get((source_name, None))
    )
...
def _operations_checks(
    ...
    for record in records:
        if "_malformed" in record:
            alerts.append(
                _alert(
                    "malformed_ingest_log",
                    f"Ingest log is malformed: {record['_malformed']}",
                    next_step="Repair the append-only log record and rerun the gate.",
                )
            )
            continue
        ...
        try:
            ...
            alerts.append(
                _alert(
                    "malformed_ingest_log",
                    f"{source_id or 'unknown source'} has an invalid retrieved_at_utc timestamp.",
                    next_step="Repair the append-only log record and rerun the gate.",
                )
            )
            continue
    ...
    try:
        ...
        alerts.append(
            _alert(
                ...
                str(exc),
                next_step="Repair the declared provenance-to-operations mapping before dashboard promotion.",
            )
        )
        mappings = {}
    ...
    for (source_name, source_version), count in sorted(
        ...
        if mapping is None:
            version_suffix = (
                f" at version {source_version!r}" if source_version is not None else ""
            )
            alerts.append(
                _alert(
                    "unoperated_source",
                    f"Curated rows cite {source_name!r}{version_suffix}, which has no operations record or declared source mapping.",
                    next_step="Add ownership, refresh, and freshness metadata before dashboard promotion.",
                )
            )
            continue
    ...
    if source_names and not records:
        alerts.append(
            _alert(
                ...
                "Curated rows have provenance but no append-only ingest log was supplied.",
                severity="warning",
                next_step="Run with --ingest-log before relying on a dashboard.",
            )
        )
    ...
    for source in operations["sources"]:
        ...
        if failed:
            alerts.append(
                _alert(
                    "failed_ingest",
                    f"{source_id} has {len(failed)} failed ingest record(s).",
                    owner=owner,
                    next_step="Investigate the latest failed source release; do not treat it as empty.",
                )
            )
        ok = [row for row in rows if row.get("status") == "ok"]
        if (
            timestamped_rows
            and max(timestamped_rows, key=lambda item: item[0])[1].get("status")
            == "partial"
        ):
            alerts.append(
                _alert(
                    "partial_ingest",
                    f"{source_id}'s latest ingest record is partial.",
                    owner=owner,
                    next_step="Complete or explicitly reject the source release before promotion.",
                )
            )
        if ok and all(int(row.get("row_count", 0)) == 0 for row in ok):
            alerts.append(
                _alert(
                    "zero_row_success",
                    f"{source_id} reported success with zero rows.",
                    owner=owner,
                    next_step="Compare the release with its prior row count and repair the fetch or parser.",
                )
            )
        if source["freshness_sla_hours"] is not None and ok:
            ...
            if age_hours > source["freshness_sla_hours"]:
                alerts.append(
                    _alert(
                        "stale_source",
                        f"{source_id} is {age_hours:.1f} hours old; SLA is {source['freshness_sla_hours']} hours.",
                        owner=owner,
                        next_step="Refresh the source or label/withhold dependent values.",
                    )
                )
        if source_id in required_operation_ids and not ok:
            alerts.append(
                _alert(
                    "source_curated_mismatch",
                    f"Curated rows cite {source_id}, but no successful ingest-log record exists.",
                    owner=owner,
                    next_step="Restore the matching ingest record or rebuild from a logged source release.",
                )
            )
        ...
        if ok and curated_count:
            ...
            if expected is None:
                alerts.append(
                    _alert(
                        "reconciliation_unavailable",
                        f"{source_id} has no curated_row_count in its latest successful ingest record.",
                        owner=owner,
                        severity="warning",
                        next_step="Record the transformed row count at ingest, then rerun the gate.",
                    )
                )
                ...
                alerts.append(
                    _alert(
                        "source_curated_mismatch",
                        f"{source_id} logged {expected} curated rows but the artifact contains {curated_count} mapped row(s).",
                        owner=owner,
                        next_step="Rebuild from the logged release or correct the append-only ingest record.",
                    )
                )
    return alerts
...
def _api_health(url: str | None) -> tuple[dict[str, str], list[dict[str, str]]]:
    ...
    if not url:
        return {
            "status": "unavailable",
            "message": "No API health URL was supplied.",
        }, []
    try:
        with urllib.request.urlopen(
            urllib.request.Request(
                url, headers={"User-Agent": "flux-quality-gate/1.0"}
            ),
            timeout=5,
        ) as response:
            ...
            raise urllib.error.HTTPError(
                url, response.status, "non-success status", response.headers, None
            )
    except (OSError, urllib.error.URLError, urllib.error.HTTPError) as exc:
        alert = _alert(
            "api_health",
            f"API health probe failed for {url}: {exc}",
            next_step="Restore the configured API health endpoint before an API-backed release.",
        )
...
def run_quality_gate(
    ...
    *,
    ...
    try:
        ...
        ingest_log = (
            _load_json_lines(Path(ingest_log_path)) if ingest_log_path else None
        )
        ...
        for table, before in previous.items():
            ...
            if after is not None and before and after < before:
                alerts.append(
                    _alert(
                        "volume_regression",
                        f"{table} fell from {before} to {after} rows.",
                        next_step="Review the release diff before promoting the curated artifact.",
                    )
                )
    else:
        alerts.append(
            _alert(
                ...
                "No reviewed expected row-count baseline was supplied.",
                next_step="Provide --previous-counts with declared minimum counts before dashboard promotion.",
            )
        )
    if not ingest_log_path:
        alerts.append(
            _alert(
                "reconciliation_unavailable",
                "No append-only ingest log was supplied.",
                next_step="Provide --ingest-log before dashboard promotion.",
            )
        )
