...
TABLES = (
    ...
    "mn_geography_artifacts",
    "mn_fixture_artifacts",
    "mn_scenario_artifacts",
    "mn_model_results",
    "mn_score_results",
    "mn_citation_chunks",
    "mn_citation_hits",
...
def ensure_minnesota_schema(con: duckdb.DuckDBPyConnection) -> None:
    ...
    mn_tables = {
        r[0] for r in con.execute("SHOW TABLES").fetchall() if r[0].startswith("mn_")
    }
    if mn_tables and existing is None:
        raise RuntimeError("Minnesota schema metadata is absent; migrate explicitly.")
    if existing is not None and existing != SCHEMA_VERSION:
        raise RuntimeError(
            f"Minnesota schema version is {existing!r}, expected {SCHEMA_VERSION!r}; migrate explicitly."
        )
    con.execute("BEGIN")
    try:
        ...
        if existing is None:
            con.execute(
                "INSERT INTO mn_schema_meta VALUES ('contract_version', ?)",
                [SCHEMA_VERSION],
            )
        _validate(con)
        con.execute("COMMIT")
...
def _validate(con: duckdb.DuckDBPyConnection) -> None:
    ...
    if missing:
        raise RuntimeError(
            f"Minnesota schema missing tables: {sorted(missing)!r}; migrate explicitly."
        )
    expected = (
        ("artifact_id", "VARCHAR", True, True),
        ("artifact_kind", "VARCHAR", True, False),
        ("contract_version", "VARCHAR", True, False),
        ("geography_id", "VARCHAR", True, False),
        ("availability", "VARCHAR", True, False),
        ("model_mode", "VARCHAR", True, False),
        ("identity_json", "JSON", True, False),
        ("created_at", "TIMESTAMP", True, False),
        ("assumptions_json", "JSON", True, False),
        ("limitations_json", "JSON", True, False),
        ("input_artifact_ids_json", "JSON", True, False),
    )
    actual_columns = tuple(
        (row[1], row[2], bool(row[3]), bool(row[5]))
        for row in con.execute("PRAGMA table_info('mn_artifact_manifests')").fetchall()
    )
    constraints = " ".join(
        str(value)
        for row in con.execute(
            "SELECT * FROM duckdb_constraints() WHERE table_name='mn_artifact_manifests'"
        ).fetchall()
        for value in row
    )
    if (
        actual_columns != expected
        or "availability IN" not in constraints
        or "model_mode IN" not in constraints
    ):
        raise RuntimeError(
            "mn_artifact_manifests shape is incompatible; migrate explicitly."
        )
    ...
    checks = (
        (
            "SELECT d.artifact_id FROM (SELECT artifact_id FROM mn_geography_artifacts UNION ALL SELECT artifact_id FROM mn_fixture_artifacts UNION ALL SELECT artifact_id FROM mn_scenario_artifacts UNION ALL SELECT artifact_id FROM mn_model_results UNION ALL SELECT artifact_id FROM mn_score_results UNION ALL SELECT corpus_artifact_id AS artifact_id FROM mn_citation_chunks UNION ALL SELECT artifact_id FROM mn_citation_hits) d JOIN mn_artifact_manifests m USING(artifact_id) WHERE m.availability <> 'available' LIMIT 1",
            "domain row requires available manifest",
        ),
        (
            "SELECT m.artifact_id FROM mn_artifact_manifests m WHERE m.availability='unavailable' AND EXISTS (SELECT 1 FROM mn_geography_artifacts d WHERE d.artifact_id=m.artifact_id) LIMIT 1",
            "unavailable artifact has domain row",
        ),
        (
            "SELECT g.artifact_id FROM mn_geography_artifacts g WHERE g.coordinate_status='unavailable' AND (g.geometry_wkb IS NOT NULL OR g.lon IS NOT NULL) LIMIT 1",
            "unavailable geometry has values",
        ),
        (
            "SELECT f.artifact_id FROM mn_artifact_field_provenance f JOIN mn_artifact_provenance p USING(artifact_id,provenance_ordinal) WHERE p.is_derived AND (f.derivation_method IS NULL OR f.derivation_method='') LIMIT 1",
            "derived provenance lacks method",
        ),
        (
            "SELECT artifact_id FROM mn_scenario_artifacts WHERE scenario_label <> 'historical_weather_stress' AND (outcome_artifact_id IS NULL OR matching_method IS NULL OR matching_method='') LIMIT 1",
            "replay lacks outcome or method",
        ),
        (
            ...
            "model mode fields incompatible",
        ),
    )
