...
def fetch_texas_lines(boundary: dict[str, Any], session: requests.Session | Any = requests) -> dict[str, Any]:
    ...
    ids_response = session.get(SERVICE_URL, params={**base, "returnIdsOnly": "true"}, timeout=60)
    ...
    for feature in features:
        ...
        assets.append({
            ...
            "geometry_precision_m": 0, "geometry_accuracy_basis": "Native archived HIFLD geometry; per-feature SOURCE/SOURCEDATE/VAL_METHOD/VAL_DATE/INFERRED remain in source sidecar.",
    ...
    artifact = {
        ...
        "coverage": [{"asset_class": "line", "scope_id": "us-tx", "status": "partial", "observed_count": len(assets), "denominator_count": None, "unavailable_count": None, "denominator_basis": "unknown", "source_scope": "Archived national HIFLD service selected by supplied Texas polygon; no owner-level Texas completeness declaration", "reason": "Native observed routes are partial/stale public overlay; endpoint labels are not terminals or edges."}],
    }
