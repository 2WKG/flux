...
def load_counties(
    con, tiger_zip: str, nri_source: str, states=None, vintage: str = "2024"
) -> int:
    path = Path(tiger_zip)
    selected_scope = states if isinstance(states, StateScope) else scope(states)
    counties = gpd.read_file(f"zip://{path}")
    selected = (
        counties[counties["STATEFP"].astype(str).str.zfill(2).isin(selected_scope.fips)]
        .copy()
        .to_crs(4326)
    )
    ...
    population_by_fips = pd.Series(
        population.to_numpy(), index=nri["STCOFIPS"].map(fips5)
    )
    ...
    frame = pd.DataFrame(
        {
            ...
            "name": selected["NAME"],
            "state": selected["STATEFP"].map(state_abbr),
    ...
    meta = pd.DataFrame(
        {
            "county_fips": selected["GEOID"].astype(str).str.zfill(5),
            "tiger_vintage": vintage,
            "aland_m2": selected["ALAND"],
            "awater_m2": selected["AWATER"],
        }
    )
    ...
    try:
        rows = replace_frame(
            ...
            "counties",
            frame,
            where=selected_scope.county_where(),
            source_name="census_tiger_county+fema_nri",
            source_ref=f"{path.name};{Path(nri_source).name}",
            source_version=f"{vintage};v1.20",
    ...
    log_artifact(
        con,
        source="census_tiger_county",
        source_release=vintage,
        path=path,
        rows_loaded=rows,
        scope_key=selected_scope.slug,
        schema_fingerprint="GEOID,NAME,STUSPS,ALAND,AWATER,geometry",
    )
    return rows
