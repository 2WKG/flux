...
def test_node_annotations_are_deterministic_and_do_not_hide_both_roles() -> None:
    ...
    con.execute("INSERT INTO buses VALUES (4, NULL, NULL), (3, '48453', 'ERCO'), (2, '48453', 'ERCO'), (1, '48453', 'ERCO')")
    con.execute("INSERT INTO counties VALUES ('48453', 'Travis')")
    con.execute("INSERT INTO gens VALUES (1, 'solar', 10), (1, 'gas', 20), (3, 'wind', 30)")
    con.execute("INSERT INTO loads VALUES (1, 5), (2, 8), (3, 0)")
    con.execute("INSERT INTO critical_loads VALUES (9, 'hospital', 'A', 1), (2, 'water', 'B', 1)")

    annotations = read_node_annotations(con)

    assert [node.bus_id for node in annotations] == [1, 2, 3, 4]
    assert [node.role for node in annotations] == ['both', 'consumer', 'producer', 'transmission']
    both = annotations[0]
    ...
    assert both.critical_loads == ({'cl_id': 2, 'name': 'B', 'kind': 'water'}, {'cl_id': 9, 'name': 'A', 'kind': 'hospital'})
