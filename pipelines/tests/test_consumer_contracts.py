...
def test_full_check_suite_is_read_only_and_preserves_fixture_hash(
    tmp_path: Path,
) -> None:
...
def test_missing_fixture_is_documented_unavailable(tmp_path: Path) -> None:
    ...
    assert all(
        result.reason is not None and "does not exist" in result.reason
...
def test_wrong_contract_version_is_named(tmp_path: Path) -> None:
    ...
    assert (
        result.reason
        == f"contract violation: invalid contract element \"schema_meta.contract_version\": expected {SCHEMA_VERSION}, found '0.0.0'"
    )
...
def test_bad_coordinate_is_an_epsg_4326_contract_violation(tmp_path: Path) -> None:
    ...
    assert (
        result.reason
        == 'contract violation: field "buses.lon" is not a valid EPSG:4326 longitude in [-180, 180]'
    )
...
def test_blank_provenance_is_a_named_contract_violation(tmp_path: Path) -> None:
    ...
    assert (
        result.reason
        == 'contract violation: field "buses.source_name" is blank or unavailable'
    )
