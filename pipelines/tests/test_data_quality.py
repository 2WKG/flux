...
def _database(tmp_path):
    ...
    ensure_schema(con)
    provenance = ["eia-930", "release-1", "v1", datetime(2026, 9, 5, tzinfo=UTC), "batch-1"]
    con.execute(
...
def _operations(tmp_path, *, sla: int | None = 48):
    ...
    path.write_text(json.dumps({"sources": [{"id": "eia-930", "owner": "data-oncall", "freshness_sla_hours": sla}]}))
    return path
...
def test_schema_enforces_null_unique_and_referential_integrity(tmp_path):
    ...
    with pytest.raises(duckdb.ConstraintException):
        con.execute("INSERT INTO counties VALUES (NULL, 'x', 'MN', 1, ?, 's', 'r', 'v', NULL, 'b')", [b"x"])
    with pytest.raises(duckdb.ConstraintException):
        con.execute("INSERT INTO counties VALUES ('27001', 'x', 'MN', 1, ?, 's', 'r', 'v', NULL, 'b')", [b"x"])
    with pytest.raises(duckdb.ConstraintException):
        con.execute(
...
def test_gate_checks_contract_values_and_volume_regressions(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("pipelines.data_quality.ACCEPTED_VALUES", {("counties", "state"): {"TX"}})
    previous = tmp_path / "previous.json"
    previous.write_text(json.dumps({"counties": 2}))
    report = run_quality_gate(path, operations, previous_counts_path=previous)
    assert {alert["code"] for alert in report["alerts"]} >= {"accepted_values", "volume_regression"}
...
def test_gate_reports_stale_failed_and_zero_row_source_records(tmp_path):
    ...
    log = _log(tmp_path, [
        {"source_id": "eia-930", "status": "ok", "row_count": 0, "retrieved_at_utc": (now - timedelta(hours=72)).isoformat()},
        {"source_id": "eia-930", "status": "failed", "row_count": 0, "retrieved_at_utc": now.isoformat()},
    ])
    baseline = tmp_path / "baseline.json"
    baseline.write_text(json.dumps({"counties": 1}))
    report = run_quality_gate(path, operations, ingest_log_path=log, previous_counts_path=baseline, now=now)
...
def test_gate_reconciles_curated_provenance_with_ingest_log(tmp_path):
    ...
    baseline = tmp_path / "baseline.json"
    baseline.write_text(json.dumps({"counties": 1}))
    report = run_quality_gate(path, operations, ingest_log_path=_log(tmp_path, []), previous_counts_path=baseline)
...
def test_gate_compares_logged_curated_count_to_the_artifact(tmp_path):
    path = _database(tmp_path)
    operations = _operations(tmp_path)
    log = _log(tmp_path, [{"source_id": "eia-930", "status": "ok", "row_count": 100, "curated_row_count": 99, "retrieved_at_utc": datetime.now(UTC).isoformat()}])
    baseline = tmp_path / "baseline.json"
    baseline.write_text(json.dumps({"counties": 1}))
...
def test_gate_can_probe_a_real_local_health_endpoint(tmp_path):
    ...
    try:
        report = run_quality_gate(
            _database(tmp_path), _operations(tmp_path),
...
def test_latest_partial_ingest_blocks_promotion(tmp_path):
    ...
    baseline = tmp_path / "baseline.json"
    baseline.write_text(json.dumps({"counties": 1}))
    log = _log(tmp_path, [{"source_id": "eia-930", "status": "partial", "row_count": 1, "retrieved_at_utc": datetime.now(UTC).isoformat()}])
    report = run_quality_gate(path, _operations(tmp_path), ingest_log_path=log, previous_counts_path=baseline)
...
def test_gate_reports_unhealthy_local_health_endpoint(tmp_path):
    ...
    thread.start()
    try:
        report = run_quality_gate(_database(tmp_path), _operations(tmp_path), api_health_url=f"http://127.0.0.1:{server.server_port}/health")
    finally:
        server.shutdown()
        thread.join()
...
def test_unknown_curated_source_and_future_log_are_blocking(tmp_path):
    ...
    baseline = tmp_path / "baseline.json"
    baseline.write_text(json.dumps({"counties": 1}))
    log = _log(tmp_path, [{"source_id": "eia-930", "status": "ok", "row_count": 1, "retrieved_at_utc": (datetime.now(UTC) + timedelta(days=1)).isoformat()}])
    report = run_quality_gate(path, _operations(tmp_path), ingest_log_path=log, previous_counts_path=baseline)
    assert {"unoperated_source", "malformed_ingest_log"} <= {alert["code"] for alert in report["alerts"]}
    assert not report["dashboard_eligible"]
...
def test_latest_timestamp_beats_log_order_for_partial_status(tmp_path):
    ...
    now = datetime(2026, 9, 5, 12, tzinfo=UTC)
    log = _log(tmp_path, [
        {"source_id": "eia-930", "status": "partial", "row_count": 1, "retrieved_at_utc": (now - timedelta(hours=1)).isoformat()},
        {"source_id": "eia-930", "status": "ok", "row_count": 1, "curated_row_count": 1, "retrieved_at_utc": (now - timedelta(hours=2)).isoformat()},
    ])
