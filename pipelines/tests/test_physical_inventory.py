...
def _artifact() -> dict:
    artifact = {
        ...
        "assets": [{"asset_id": "eia860:plant:1", "asset_class": "generation", "asset_kind": "plant", "source_id": "eia860", "source_record_id": "1", "geometry": {"type": "Point", "coordinates": [-97.0, 32.0]}, "geometry_crs": "EPSG:4326", "geometry_precision_m": 10.0, "geometry_accuracy_basis": "source-reported plant coordinate", "geometry_derivation_method": None, "geometry_status": "source"}],
        "terminals": [],
        "connectivity_edges": [],
        "coverage": [{"asset_class": "generation", "scope_id": "tx", "status": "partial", "observed_count": 1, "denominator_count": None, "unknown_count": None, "unavailable_count": None, "denominator_basis": "unknown", "source_scope": "EIA-860 reported Texas plants", "reason": "The source reports observed plants but no statewide completeness denominator."}, {"asset_class": "line", "scope_id": "tx", "status": "unavailable", "observed_count": 0, "denominator_count": None, "unknown_count": None, "unavailable_count": None, "denominator_basis": "unknown", "source_scope": "Texas statewide", "reason": "No source artifact was acquired for this release."}],
    }
...
def test_rejects_coverage_omissions_and_unavailable_geometry_metadata() -> None:
    ...
    asset.update({"geometry": None, "geometry_crs": "EPSG:4326", "geometry_precision_m": 0.01, "geometry_accuracy_basis": "invented", "geometry_status": "unavailable"})
    artifact["content_sha256"] = artifact_sha256(artifact)
    with pytest.raises(PhysicalInventoryError, match="unavailable geometry"):
        validate_artifact(artifact)


def test_rejects_missing_asset_coverage_overcount_and_unproven_derived_geometry() -> None:
    artifact = _artifact()
    artifact["coverage"] = [artifact["coverage"][1]]
    artifact["content_sha256"] = artifact_sha256(artifact)
    with pytest.raises(PhysicalInventoryError, match="required for observed asset classes"):
        validate_artifact(artifact)
    artifact = _artifact()
    row = artifact["coverage"][0]
    row.update({"status": "complete", "denominator_count": 1, "unknown_count": 0, "unavailable_count": 0, "observed_count": 2})
    artifact["content_sha256"] = artifact_sha256(artifact)
