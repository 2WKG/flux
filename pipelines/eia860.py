...
def _latest_generator_reports(inventory: pd.DataFrame) -> pd.DataFrame:
    ...
    return inventory.sort_values(
        ["plant_id_eia", "generator_id", "report_date"], kind="stable"
...
def _county_fips_for_points(con, lon: pd.Series, lat: pd.Series) -> list[str | None]:
    ...
    shapes = [
        (county_fips, from_wkb(bytes(geom_wkb))) for county_fips, geom_wkb in counties
    ]
    return [
        next((fips for fips, shape in shapes if shape.covers(Point(x, y))), None)
        for x, y in zip(lon, lat, strict=True)
    ]


def _candidate_bus_ids(
    con, candidates: pd.DataFrame, min_kv: float = 230.0
) -> pd.Series:
    ...
    for row in candidates.itertuples():
        ...
        pool = pool.assign(_distance_sq=distance_sq).sort_values(
            ["_distance_sq", "bus_id"], kind="stable"
        )
...
def load_eia860_plants(
    con,
    plants_parquet: str,
    generators_parquet: str,
    release: str = "v2026.2.0",
    states=None,
) -> int:
    ...
    generators["report_date"] = pd.to_datetime(
        generators["report_date"], errors="coerce"
    ).dt.date
    generators["capacity_mw"] = pd.to_numeric(
        generators["capacity_mw"], errors="coerce"
    )
    generators["fuel_type_code_pudl"] = _first(generators, "fuel_type_code_pudl")
    generators["retirement_date"] = pd.to_datetime(
        _first(generators, "generator_retirement_date"), errors="coerce"
    ).dt.date
    generators["planned_retirement_date"] = pd.to_datetime(
        _first(generators, "planned_generator_retirement_date"), errors="coerce"
    ).dt.date
    inventory = pd.DataFrame(
        {
            "plant_id_eia": generators["plant_id_eia"],
            "generator_id": generators["generator_id"].astype(str),
            "report_date": generators["report_date"],
            "capacity_mw": generators["capacity_mw"],
            ...
            "retirement_date": generators["retirement_date"],
            "planned_retirement_date": generators["planned_retirement_date"],
        }
    ...
    plant_dates["report_date"] = pd.to_datetime(
        plant_dates["report_date"], errors="coerce"
    )
    latest = (
        plant_dates.sort_values("report_date")
        .groupby("plant_id_eia", as_index=False)
        .tail(1)
    )
    latest_gen = _latest_generator_reports(inventory).groupby("plant_id_eia")
    aggregate = latest_gen.agg(
        capacity_mw=("capacity_mw", "sum"),
        primary_fuel=("fuel_type_code_pudl", _fuel_mode),
        retirement_year=(
            "retirement_date",
            lambda values: pd.to_datetime(values, errors="coerce").dt.year.min(),
        ),
        ...
        operational_status=(
            "operational_status",
            lambda values: (
                values.dropna().mode().iat[0] if not values.dropna().empty else pd.NA
            ),
        ),
    )
    curated = latest.merge(aggregate, on="plant_id_eia", how="left")
    curated = _scope_plants(curated, selected_scope)
    curated["retirement_year"] = curated["retirement_year"].combine_first(
        curated["planned_retirement_year"]
    )
    plant_frame = pd.DataFrame(
        {
            "plant_id_eia": curated["plant_id_eia"].astype(int),
            "plant_name": _first(curated, "plant_name_eia").astype(str),
            "lon": pd.to_numeric(curated["longitude"], errors="coerce"),
            "lat": pd.to_numeric(curated["latitude"], errors="coerce"),
            "state": curated["state"],
            "county_fips": _county_fips_for_points(
                con, curated["longitude"], curated["latitude"]
            ),
            "capacity_mw": curated["capacity_mw"],
            "primary_fuel": curated["primary_fuel"],
            "retirement_year": curated["retirement_year"].astype("Int64"),
            "operational_status": curated["operational_status"],
            "report_date": pd.to_datetime(curated["report_date"]).dt.date,
        }
...
def seed_site_candidates(con, states=None) -> int:
    ...
    selected_scope = scope(states)
    plants = con.execute(
        f"SELECT * FROM eia_plants WHERE {_state_where(selected_scope)}"
    ).fetchdf()
    inventory = con.execute("SELECT * FROM eia_generator_inventory").fetchdf()
    latest_units = _latest_generator_reports(inventory)
    latest_units["fuel_type_code_pudl"] = (
        latest_units["fuel_type_code_pudl"].astype("string").str.lower()
    )
    latest_units["operational_status"] = (
        latest_units["operational_status"].astype("string").str.lower()
    )
    ...
    coal_units["has_near_term_plan"] = pd.to_datetime(
        coal_units["planned_retirement_date"], errors="coerce"
    ).dt.year.le(2032)
    ...
    nuclear_candidates = plants[plants.primary_fuel.eq("nuclear")][
        ["plant_id_eia"]
    ].copy()
    nuclear_candidates["kind"] = "nuclear_existing"
    candidate_kinds = pd.concat(
        [coal_candidates, nuclear_candidates], ignore_index=True
    )
    ...
    selected = selected.sort_values("plant_id_eia", kind="stable").drop_duplicates(
        "plant_id_eia", keep="first"
    )
    ...
    candidates["source_site_id"] = (
        "eia_plant:" + selected["plant_id_eia"].astype(str).to_numpy()
    )
