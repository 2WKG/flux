...
def _timestamp(value: Any, field: str) -> datetime:
    if not isinstance(value, str):
        raise PhysicalInventoryError(f"{field} must be an ISO-8601 timestamp")
    try:
        parsed = datetime.fromisoformat(value)
    except ValueError as exc:
        raise PhysicalInventoryError(f"{field} is not ISO-8601") from exc
    if parsed.tzinfo is None:
        raise PhysicalInventoryError(f"{field} must include an offset")
    return parsed.astimezone(UTC).replace(tzinfo=None)


def _sha(value: Any, field: str) -> None:
    if not isinstance(value, str) or re.fullmatch(r"[0-9a-f]{64}", value) is None:
        raise PhysicalInventoryError(f"{field} must be a lowercase SHA-256")
    ...
    if status == "unavailable":
        if value is not None:
            raise PhysicalInventoryError(
                f"{prefix}.geometry must be null when unavailable"
            )
        return
    if not isinstance(value, dict) or value.get("type") not in GEOMETRY_TYPES:
        raise PhysicalInventoryError(
            f"{prefix}.geometry must be a non-empty GeoJSON physical geometry"
        )
    try:
        geom = shape(value)
    except Exception as exc:
        raise PhysicalInventoryError(f"{prefix}.geometry is invalid GeoJSON") from exc
    if geom.is_empty or not geom.is_valid:
        raise PhysicalInventoryError(f"{prefix}.geometry must be valid and non-empty")
    west, south, east, north = geom.bounds
    if (
        not all(math.isfinite(value) for value in (west, south, east, north))
        or crs == "EPSG:4326"
        and not (-180 <= west <= east <= 180 and -90 <= south <= north <= 90)
    ):
        raise PhysicalInventoryError(
            f"{prefix}.geometry has invalid EPSG:4326 coordinates"
        )
...
def _registered_crs(value: Any, prefix: str) -> None:
    ...
    if not isinstance(value, str) or re.fullmatch(r"(?:EPSG|ESRI):\d+", value) is None:
        raise PhysicalInventoryError(
            f"{prefix}.geometry_crs must be an explicit registered EPSG or ESRI CRS"
        )
    ...
    try:
        ...
        raise PhysicalInventoryError(
            f"{prefix}.geometry_crs is not a registered CRS"
        ) from exc
    if resolved != (authority, code):
        raise PhysicalInventoryError(
            f"{prefix}.geometry_crs must retain its declared authority"
        )
...
def validate_artifact(artifact: dict[str, Any]) -> dict[str, Any]:
    ...
    if not required <= set(artifact) or set(artifact) - allowed:
        raise PhysicalInventoryError(
            f"artifact fields must include {sorted(required)!r} and only known optional fields"
        )
    ...
    if (
        ...
        raise PhysicalInventoryError(
            "geography_id and semantic artifact_version are required"
        )
    if artifact["artifact_id"] != f"{geo}:physical-inventory:{version}":
        raise PhysicalInventoryError(
            "artifact_id must be '<geography_id>:physical-inventory:<version>'"
        )
    if (
        artifact["inventory_mode"] not in INVENTORY_MODES
        or artifact["electrical_model_mode"] not in MODEL_MODES
    ):
        raise PhysicalInventoryError("invalid inventory or electrical model mode")
    _timestamp(artifact["created_at"], "created_at")
    _sha(artifact["content_sha256"], "content_sha256")
    if artifact["content_sha256"] != artifact_sha256(artifact):
        raise PhysicalInventoryError("content_sha256 does not match canonical artifact")
    if "input_artifact_sha256s" in artifact and (
        not isinstance(artifact["input_artifact_sha256s"], list)
        or not artifact["input_artifact_sha256s"]
        or artifact["input_artifact_sha256s"]
        != sorted(set(artifact["input_artifact_sha256s"]))
    ):
        raise PhysicalInventoryError(
            "input_artifact_sha256s must be a sorted unique non-empty array"
        )
    for index, digest in enumerate(artifact.get("input_artifact_sha256s", [])):
        _sha(digest, f"input_artifact_sha256s[{index}]")
    sources = artifact["sources"]
    if not isinstance(sources, list) or not sources:
        raise PhysicalInventoryError("sources must be a non-empty array")
    source_ids = set()
    for i, row in enumerate(sources):
        if set(row) != {
            ...
            raise PhysicalInventoryError(f"sources[{i}] has incompatible fields")
        if (
            not all(
            ...
            raise PhysicalInventoryError(f"sources[{i}] has invalid identity")
        source_ids.add(row["source_id"])
        _timestamp(row["retrieved_at"], f"sources[{i}].retrieved_at")
        _sha(row["content_sha256"], f"sources[{i}].content_sha256")
    assets = artifact["assets"]
    if not isinstance(assets, list):
        raise PhysicalInventoryError("assets must be an array")
    asset_ids = set()
    for i, row in enumerate(assets):
        ...
        if set(row) != need:
            raise PhysicalInventoryError(f"assets[{i}] has incompatible fields")
        if (
            not all(
                isinstance(row[k], str) and row[k]
                for k in (
            ...
            or row["asset_class"] not in ASSET_CLASSES
            or row["source_id"] not in source_ids
        ):
            raise PhysicalInventoryError(
                f"assets[{i}] has invalid identity/class/source"
            )
        asset_ids.add(row["asset_id"])
        if row["geometry_status"] not in {"source", "derived", "unavailable"}:
            raise PhysicalInventoryError(f"assets[{i}] invalid geometry_status")
        if row["geometry_status"] == "unavailable" and any(
            row[key] is not None
            for key in (
                "geometry_crs",
                "geometry_precision_m",
            ...
            raise PhysicalInventoryError(
                f"assets[{i}] unavailable geometry must not have CRS, precision, accuracy, or derivation values"
            )
        if row["geometry_status"] != "unavailable" and (
            row["geometry_precision_m"] is not None
            and (
                not isinstance(row["geometry_precision_m"], (int, float))
                or isinstance(row["geometry_precision_m"], bool)
                or row["geometry_precision_m"] < 0
            )
            or not isinstance(row["geometry_accuracy_basis"], str)
            or not row["geometry_accuracy_basis"]
        ):
            raise PhysicalInventoryError(
                f"assets[{i}] needs an accuracy basis; numeric precision may be null only when unknown"
            )
        if row["geometry_status"] != "unavailable":
            _registered_crs(row["geometry_crs"], f"assets[{i}]")
        if row["geometry_status"] == "derived" and (
            not isinstance(row["geometry_derivation_method"], str)
            or not row["geometry_derivation_method"]
        ):
            raise PhysicalInventoryError(
                f"assets[{i}] derived geometry needs a derivation method"
            )
        if (
            row["geometry_status"] == "source"
            and row["geometry_derivation_method"] is not None
        ):
            raise PhysicalInventoryError(
                f"assets[{i}] source geometry must not claim a derivation method"
            )
        _geometry(
            row["geometry"], row["geometry_status"], row["geometry_crs"], f"assets[{i}]"
        )
    if (
        not isinstance(artifact["terminals"], list)
        or not isinstance(artifact["connectivity_edges"], list)
        or not isinstance(artifact["coverage"], list)
    ):
        raise PhysicalInventoryError(
            "terminals, connectivity_edges, and coverage must be arrays"
        )
    terminal_ids = set()
    for i, row in enumerate(artifact["terminals"]):
        if (
            set(row) != {"terminal_id", "asset_id", "source_id", "source_record_id"}
            or not all(isinstance(row[k], str) and row[k] for k in row)
            or row["terminal_id"] in terminal_ids
            or row["asset_id"] not in asset_ids
            or row["source_id"] not in source_ids
        ):
            raise PhysicalInventoryError(
                f"terminals[{i}] must be a source-backed terminal for an asset"
            )
    ...
    for i, row in enumerate(artifact["connectivity_edges"]):
        if (
            ...
            or row["from_terminal_id"] not in terminal_ids
            or row["to_terminal_id"] not in terminal_ids
            or row["from_terminal_id"] == row["to_terminal_id"]
            or row["source_id"] not in source_ids
        ):
            raise PhysicalInventoryError(
                f"connectivity_edges[{i}] must join two sourced distinct terminals"
            )
    ...
    for i, row in enumerate(artifact["coverage"]):
        ...
        if (
            set(row) != need
            or row["asset_class"] not in ASSET_CLASSES
            or not isinstance(row["scope_id"], str)
            or not row["scope_id"]
            or row["status"] not in COVERAGE_STATUSES
            or not isinstance(row["observed_count"], int)
            or isinstance(row["observed_count"], bool)
            or row["observed_count"] < 0
            or any(
                row[key] is not None
                and (
                ...
                for key in ("unknown_count", "unavailable_count")
            )
            or not all(
                isinstance(row[k], str) and row[k]
                for k in ("denominator_basis", "source_scope", "reason")
            )
        ):
            raise PhysicalInventoryError(
                f"coverage[{i}] has invalid class/status/counts"
            )
        denom = row["denominator_count"]
        if denom is not None and (
            not isinstance(denom, int) or isinstance(denom, bool) or denom < 0
        ):
            raise PhysicalInventoryError(
                f"coverage[{i}].denominator_count must be non-negative or null"
            )
        if row["status"] == "complete" and (
            denom is None
            or row["unknown_count"] != 0
            or row["unavailable_count"] != 0
            or row["observed_count"] != denom
        ):
            raise PhysicalInventoryError(
                f"coverage[{i}] cannot claim complete without exact reconciled counts"
            )
        key = (row["asset_class"], row["scope_id"])
        if key in coverage_keys:
            raise PhysicalInventoryError(f"coverage[{i}] duplicates class/scope")
        coverage_keys.add(key)
    if not coverage_keys:
        raise PhysicalInventoryError("coverage must declare at least one class/scope")
    missing_classes = {row["asset_class"] for row in assets} - {
        key[0] for key in coverage_keys
    }
    if missing_classes:
        raise PhysicalInventoryError(
            f"coverage is required for observed asset classes: {sorted(missing_classes)!r}"
        )
    return artifact
...
def ensure_physical_inventory_schema(con: duckdb.DuckDBPyConnection) -> None:
    ...
    if "physical_inventory_schema_meta" in existing:
        value = con.execute(
            "SELECT value FROM physical_inventory_schema_meta WHERE key='contract_version'"
        ).fetchone()
        if value is None or value[0] != CONTRACT_VERSION:
            raise RuntimeError(
                "physical inventory schema requires an explicit migration"
            )
    elif any(name.startswith("physical_") for name in existing):
        raise RuntimeError(
            "physical inventory schema metadata is absent; migrate explicitly"
        )
    con.execute("BEGIN")
    try:
        ...
        if "physical_inventory_schema_meta" not in existing:
            con.execute(
                "INSERT INTO physical_inventory_schema_meta VALUES ('contract_version', ?)",
                [CONTRACT_VERSION],
            )
        con.execute("COMMIT")
    except Exception:
...
def write_artifact(con: duckdb.DuckDBPyConnection, artifact: dict[str, Any]) -> str:
    ...
    existing = con.execute(
        "SELECT content_sha256 FROM physical_inventory_manifests WHERE artifact_id=?",
        [aid],
    ).fetchone()
    if existing:
        if existing[0] != artifact["content_sha256"]:
            raise PhysicalInventoryError(
                f"artifact {aid!r} conflicts with persisted content"
            )
        return aid
    con.execute("BEGIN")
    try:
        con.execute(
            "INSERT INTO physical_inventory_manifests VALUES (?,?,?,?,?,?,?,?,?)",
            [
                aid,
                ...
                _canonical(artifact),
                artifact["inventory_mode"],
                artifact["electrical_model_mode"],
                _timestamp(artifact["created_at"], "created_at"),
        ...
        for row in artifact["sources"]:
            con.execute(
                "INSERT INTO physical_inventory_sources VALUES (?,?,?,?,?,?,?,?)",
                [
                    aid,
                    row["source_id"],
                    row["authority"],
                    row["source_ref"],
                    row["source_version"],
                    _timestamp(row["retrieved_at"], "retrieved_at"),
        ...
        for row in artifact["assets"]:
            con.execute(
                "INSERT INTO physical_assets VALUES (?,?,?,?,?,?,?,?,?,?,?,?)",
                [
                    aid,
                    row["asset_id"],
                    row["asset_class"],
                    ...
                    _canonical(row["geometry"])
                    if row["geometry"] is not None
                    else None,
                    row["geometry_crs"],
                    row["geometry_precision_m"],
                    row["geometry_accuracy_basis"],
                    row["geometry_derivation_method"],
                    row["geometry_status"],
                ],
            )
        for row in artifact["terminals"]:
            con.execute(
                "INSERT INTO physical_asset_terminals VALUES (?,?,?,?,?)",
                [
                    aid,
                    row["terminal_id"],
                    row["asset_id"],
        ...
        for row in artifact["connectivity_edges"]:
            con.execute(
                "INSERT INTO physical_connectivity_edges VALUES (?,?,?,?,?,?)",
                [
                    aid,
                    row["edge_id"],
                    row["from_terminal_id"],
                    row["to_terminal_id"],
                    row["source_id"],
                    row["source_record_id"],
                ],
        ...
        for row in artifact["coverage"]:
            con.execute(
                "INSERT INTO physical_coverage VALUES (?,?,?,?,?,?,?,?,?,?,?)",
                [
                    aid,
                    row["asset_class"],
                    row["scope_id"],
