...
def validate_artifact(artifact: dict[str, Any]) -> dict[str, Any]:
    ...
    for i,row in enumerate(assets):
        ...
        if row["geometry_status"] != "unavailable" and (not isinstance(row["geometry_crs"],str) or re.fullmatch(r"(?:EPSG|ESRI):\d+",row["geometry_crs"]) is None or row["geometry_precision_m"] is not None and (not isinstance(row["geometry_precision_m"],(int,float)) or isinstance(row["geometry_precision_m"],bool) or row["geometry_precision_m"] < 0) or not isinstance(row["geometry_accuracy_basis"],str) or not row["geometry_accuracy_basis"]): raise PhysicalInventoryError(f"assets[{i}] needs an explicit EPSG or ESRI CRS and accuracy basis; numeric precision may be null only when unknown")
