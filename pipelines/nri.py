...
def _county_csv(path: Path) -> pd.DataFrame:
    with zipfile.ZipFile(path) as archive:
        names = [
            ...
            if name.lower().endswith(".csv") and "count" in name.lower()
        ]
        if not names:
...
def _county_records(path: Path) -> pd.DataFrame:
    ...
    records = [
        feature.get("attributes", feature) for feature in payload.get("features", [])
    ]
    if not records:
    ...
    *,
    ...
    if state is not None:
        if states is not None:
            raise ValueError("pass state or states, not both")
        states = state
    selected_scope = states if isinstance(states, StateScope) else scope(states)
    source = _county_records(path)
    selected = source[
        source["STATEABBRV"].isin(selected_scope.source_values("fema_nri"))
    ].copy()
    selected["county_fips"] = selected["STCOFIPS"].map(fips5)
    if (
        selected.county_fips.isna().any()
        or not selected.county_fips.str[:2].isin(selected_scope.fips).all()
    ):
    ...
    try:
        replace_frame(
            ...
            hazard,
            where=state_where,
            source_name="fema_nri",
            source_ref=path.name,
            source_version=release,
            fixture_batch_id=f"p0-nri-{release}-{selected_scope.slug}",
        )
        # Population belongs in the canonical county table, while the source remains in NRI provenance.
        population = pd.DataFrame(
            {
                "county_fips": selected.county_fips,
                "pop": _number(selected, "POPULATION").astype("Int64"),
            }
        )
        con.register("_nri_pop", population)
        try:
            con.execute(
                "UPDATE counties AS c SET pop = p.pop FROM _nri_pop AS p WHERE c.county_fips = p.county_fips"
            )
        finally:
                ...
                pd.DataFrame(
                    {
                        "county_fips": selected.county_fips,
                        "hazard_code": code,
                        "risk_score": _number(selected, score),
                        "risk_rating": selected.get(rating),
                        "eal_value": _number(selected, eal),
        ...
        replace_frame(
            ...
            "nri_hazards",
            helper,
            where=f"source_release = '{escaped_release}' AND ({state_where})",
        )
        con.execute("COMMIT")
    except Exception:
