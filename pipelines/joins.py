...
def join_bus_county(con, fallback_km: float = 30.0) -> int:
    ...
    county_geo = gpd.GeoDataFrame(
        counties[["county_fips"]].rename(
            columns={"county_fips": "_matched_county_fips"}
        ),
    ...
    hit = gpd.sjoin(
        bus_geo,
        county_geo[["_matched_county_fips", "geometry"]],
        how="left",
    ...
    hit = hit.sort_values(
        ["bus_id", "_matched_county_fips"], kind="stable"
    ).drop_duplicates("bus_id", keep="first")
    unmatched = hit.loc[hit._matched_county_fips.isna(), ["bus_id", "geometry"]]
    if not unmatched.empty:
        nearest = gpd.sjoin_nearest(
            unmatched.to_crs(3083),
            county_geo[["_matched_county_fips", "geometry"]].to_crs(3083),
            how="left",
            max_distance=fallback_km * 1000,
            distance_col="fallback_m",
        )
        nearest = nearest.sort_values(
            ["bus_id", "fallback_m", "_matched_county_fips"], kind="stable"
        )
        nearest = nearest.drop_duplicates("bus_id", keep="first")
        nearest_county = nearest.set_index("bus_id")._matched_county_fips
        hit["_matched_county_fips"] = hit["_matched_county_fips"].fillna(
            hit["bus_id"].map(nearest_county)
        )
    result = hit[["bus_id", "_matched_county_fips"]].rename(
        columns={"_matched_county_fips": "county_fips"}
    )
    con.register("_bus_county", result)
    try:
        con.execute(
            "UPDATE buses AS b SET county_fips = m.county_fips FROM _bus_county AS m WHERE b.bus_id = m.bus_id AND b.county_fips IS DISTINCT FROM m.county_fips"
        )
    finally:
    ...
    if missing:
        con.execute(
            "INSERT INTO ingest_warnings VALUES ('joins', 'bus_county', ?, current_timestamp)",
...
def join_critical_loads_to_bus(con, min_kv: float = 115.0) -> int:
    facilities = con.execute(
        "SELECT cl_id, county_fips, lon, lat FROM critical_loads"
    ).fetchdf()
    buses = con.execute(
        "SELECT bus_id, county_fips, base_kv, lon, lat FROM buses WHERE base_kv >= ?",
        [min_kv],
    ).fetchdf()
    if facilities.empty:
        return 0
    ...
    try:
        con.execute(
            "DELETE FROM critical_load_bus_dist WHERE cl_id IN (SELECT cl_id FROM _critical_facilities)"
        )
    finally:
    ...
    for row in facility_geo.itertuples():
        ...
        nearest = (
            pool.loc[distances.eq(minimum)].sort_values("bus_id", kind="stable").iloc[0]
        )
        matches.append(
            {
                "cl_id": row.cl_id,
                "bus_id": int(nearest.bus_id),
                "distance_km": float(minimum / 1000),
                "match_method": method,
            }
        )
    result = pd.DataFrame(matches).astype(
        {
            "cl_id": "int64",
            "bus_id": "Int64",
            "distance_km": "float64",
    ...
    try:
        ...
        con.execute(
            "INSERT OR REPLACE INTO critical_load_bus_dist SELECT * FROM _critical_bus"
        )
    finally:
