...
with the line-upgrade ranker (Idea 3) as one screen inside it and data-center load verification
(Idea 2) as a separate backup pitch. The shared contract (repo layout, DuckDB tables, scenario IDs,
copilot tool signatures) lives in `00-overview.md` §2 and wins over any downstream spec.
...
| 05 | [`05-copilot.md`](05-copilot.md) | `copilot/` | FastAPI host: thin read APIs for the map and the Claude (`claude-sonnet-5`) tool loop over `predict_outage`, `run_cascade`, `score_site`, `top_lines`, `sql`, `cite`; regulatory-PDF retrieval; SSE `/ask`; the model narrates, never computes. |
...
| 08 | [`08-line-upgrade-screen.md`](08-line-upgrade-screen.md) | line scoring + one web screen | Idea 3 inside the twin: IEEE 738 DLR uplift from county wind, reconductor uplift and REFA costs, congestion proxy from twin loading, MW-per-$ ranking, FERC RM24-6 screen, SPARK flag; writes `line_upgrade_scores`; serves `top_lines`. |
| 09 | [`09-backup-idea2-datacenter-load.md`](09-backup-idea2-datacenter-load.md) | `dc_*` tables, `copilot/tools/dc.py` | Backup pitch: data-center project registry, entity resolution/duplicate detection, reality model with intervals, Grid Impact Score (0–100, weighted 7 components, "what makes this a 90"), stranded-cost model, tools `phantom_ratio`/`duplicates`/`score_dc_site`/`cost_exposure`. |
