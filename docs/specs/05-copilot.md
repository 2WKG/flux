...
| `GET /predictions` (2WKG-104) | `scenario_id?`, `county_fips?` (`^\d{5}$`), `model_kind? ∈ {lightgbm, heuristic}`, `limit=1000` (1–1000) | bare array `[{scenario_id, county_fips, ts, p_out, customers_at_risk, driver, model_kind, model_version, artifact_sha256, split_id, feature_set_version, evaluation_sha256, rule_id, rule_version, persisted_at, evaluation_status, qualified, qualification_reason}]` — only rows whose cited evaluation is persisted `qualified = TRUE` (filtered in SQL before `LIMIT` via `models.outage.persistence.query_predictions(qualified_only=True)`), ordered `(scenario_id, county_fips, ts)`. Heuristic rows cite no evaluation and are by design never qualified: an artifact with only unqualified/heuristic rows is `503 unavailable` reason `no_qualified_prediction`; absent DB / absent `outage_predictions`·`prediction_provenance`·`evaluation_artifacts` table / drifted columns are `database_missing` / `missing` / `schema_mismatch`; malformed params are the shared 422 |
| `GET /cascade` (2WKG-104) | `scenario_id` (required), `run_id?` | one persisted `cascade_runs` run, unwrapped: `{run_id, scenario_id, hours:[{hour, tripped_element_ids, lost_load_mw, counties_dark, critical_loads_lost}], provenance:{source_name, source_ref, source_version, source_retrieved_at, fixture_batch_id, source_kind, topology}, attributes}` — `lost_load_mw` is MW (`attributes` carries unit/source per field); `topology` is `"synthetic (ACTIVSg2000)"` only when the row's own provenance references ACTIVSg, else `null` (never a constant). Without `run_id` the run with the greatest `source_retrieved_at` wins (NULLs last, then lexical `run_id` desc — `cascade_runs` has no run timestamp). No run for the scenario → `503` reason `topology_cascade_unsupported_or_absent`; `run_id` not persisted → `404 not_found`; table absent / column drift / NULL `lost_load_mw` → `503` reason `missing` / `schema_mismatch` (never a 500, never an invented number) |
