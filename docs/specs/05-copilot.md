...
The Minnesota `GET` routes above are read-only artifact retrieval. Their success
bodies carry the `status: "available"` discriminator pinned by `00-overview.md` §4.2 —
that is still an unwrapped route payload in `copilot/api/envelope.py` terms, since the
envelope is the four-field `{status, data, error, meta}` failure shape.
`GET /cascade` returns only the latest persisted run for a scenario
