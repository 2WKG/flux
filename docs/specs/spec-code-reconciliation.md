...
| **D-11** | The `/ask` answer path: `05-copilot.md` §Providers said "tool orchestration stays deployment-injected (`ask_backend`)", `texas-demo-narrative-ia.md:248` said "nothing in the repository injects one", and D-5b above records the same thing. Read together they describe a transport that is *waiting* for a deployment — but no deployment surface exists, so the state was permanent | **Proved at master `38367c6` from a clean clone, not inferred.** `create_app` built a narration provider and left `app.state.ask_backend = None`; `copilot/routes/ask.py` branches on that before any provider is reached, so with `COPILOT_PROVIDER=gemini` and a key set — `/health` reporting `provider gemini`, `model gemini-3.8-flash`, `status not_verified` — `POST /ask` still returned `lifecycle{started}` then `error{unavailable,"The local Copilot backend is not configured."}`. `grep -rn 'ask_backend=' copilot/` found constructions **only in tests**. The chat dock could not answer under any configuration | **2WKG-230: the orchestration is configuration, not injection.** `copilot/agent/` now holds the registry (`registry.py`: frozen tool name → production executor + spec 05's per-tool timeout) and the loop (`loop.py`), and `create_app` assigns `build_ask_backend(settings, provider)` to `app.state.ask_backend`. `05-copilot.md` §Providers and the Texas IA row were amended; D-5b's "nothing injects one" is superseded by this row. Four consequences are load-bearing and each has a probe that goes red without it: (1) a frozen tool with **no production executor is not registered and never advertised** to the planner — today that is `top_lines` and `causal_query` only, so the planner cannot choose a tool this deployment could not then run; (2) the model's tool arguments are **re-validated against the frozen input model** before any executor runs, never repaired; (3) every way the loop fails to find evidence is a **named unavailable narration**, so it reaches the browser as one terminal `error`, never as prose; (4) `build_ask_backend` returns `None` when no provider is configured **or** when the provider cannot plan, so a keyless deployment emits the byte-identical terminal it emitted before this change. **Step bound:** the `AskBackend` contract carries one `ToolTurn` per attempt, so the loop performs exactly one tool step; spec 05 §"Tool-use loop"'s `MAX_ITER = 8` needs a wider backend contract and is **FU-6**, named not done. **FU-7:** bind the remaining seven frozen tools (`predict_outage`, `run_cascade`, `score_site`, `sql`, `cite`, `compare_interventions`, `top_critical_elements`) — each needs a production executor returning its `ToolOutput`, or a deployment-owned registration (`sql` needs approved views, `cite` needs a corpus artifact id), neither of which exists in `Settings` today. **Unproven here:** there is no API key in this checkout, so no live model call was made. The planning turn is proved against both real SDKs over an httpx mock transport replaying recorded fixtures (`copilot/providers/test_tool_selection.py`), and the answer path end to end with a stubbed provider at the seam (`copilot/agent/test_ask_backend.py`); that a real Gemini or Claude turn chooses a sensible tool is **not** proved and must not be claimed until it is run with a key. |
