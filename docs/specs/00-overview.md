...
GET  /cascade?scenario_id=&run_id=                    → {status:"available", run_id, scenario_id, artifact_id, model_mode:"topology", geography_id, hours:[{hour, lost_load_mw (MW), …}], provenance:[…], limitations:[…], source_kind, topology, attributes} (05 §Routes, 2WKG-104)
...
GET  /predictions?scenario_id=&county_fips=&model_kind=&limit=1000 → {status:"available", predictions:[qualified persisted rows]} — filtered in SQL before LIMIT (05 §Routes, 2WKG-104)
...
The additive Minnesota `GET` routes are persisted-artifact reads. Their success
bodies keep the `status: "available"` discriminator this section pins; the four-field
failure envelope (`{status, data, error, meta}`) is what wraps a failure. Every
response also carries `X-Request-ID` and `X-Flux-Api-Version: v1`, and the qualified
`GET /cascade` success carries `X-Flux-Artifact` equal to its payload's `artifact_id`
(2WKG-294). `GET /cascade` selects a persisted cascade only when its model result is
validated, its manifest is available with
