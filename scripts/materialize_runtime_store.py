...
def _require_hash(path: Path, expected: object, label: str) -> str:
    ...
    if actual != expected:
        raise MaterializationError(f"{label} SHA-256 mismatch: expected {expected}, got {actual}")
    return actual
...
def _scenario_windows(con: duckdb.DuckDBPyConnection) -> list[tuple[str, datetime, datetime]]:
    tables = {row[0] for row in con.execute("SHOW TABLES").fetchall()}
    required = {"counties", "weather_hourly", "weather_source_runs"}
    if missing := required - tables:
        raise MaterializationError(f"weather input lacks required tables: {sorted(missing)}")
    windows = con.execute(
    ...
    if present != expected:
        raise MaterializationError(f"weather scenarios must be {sorted(expected)}, got {sorted(present)}")
    if any(row[3] != 254 or row[1] is None or row[2] is None for row in windows):
        raise MaterializationError("weather windows do not cover 254 counties at each retained scenario")
...
def _load_release(path: Path) -> dict[str, Any]:
    try:
        ...
        raise MaterializationError(f"unreadable published inventory release: {path}") from exc
...
def _derived_rows(path: Path) -> dict[str, int]:
    ...
    try:
        ...
        return {
            ...
            for table in ("cascade_runs", "outage_predictions", "site_scores", "line_upgrade_scores")
            if table in tables and con.execute(f"SELECT count(*) FROM {table}").fetchone()[0]
        }
    finally:
        con.close()


def _validate_output(con: duckdb.DuckDBPyConnection, expected_assets: int) -> dict[str, int]:
    counts = dict(
        zip(
            ("buses", "lines", "transformer_branches", "weather_hourly", "weather_source_runs", "scenarios", "physical_releases", "physical_assets"),
            con.execute("""SELECT
    ...
    if counts["buses"] != 2000 or counts["lines"] != 3206 or counts["transformer_branches"] != 847:
    ...
    if counts["scenarios"] != 2 or counts["physical_releases"] != 2 or counts["physical_assets"] != expected_assets:
        raise MaterializationError(f"runtime store is incomplete: {counts}")
    return counts


def materialize(
    *, hrrr_db: Path, hrrr_receipt: Path, aux: Path, case: Path,
    activsg_receipt: Path, inventory_root: Path, version: str, output: Path,
    receipt_output: Path, replace: bool = False, discard_derived: bool = False,
) -> dict[str, Any]:
    ...
    hrrr_hash = _require_hash(hrrr_db, _receipt_hash(hrrr_metadata, "grid.duckdb"), "HRRR DB")
    aux_hash = _require_hash(aux, _receipt_hash(activsg_metadata, aux.name), "ACTIVS AUX")
    case_hash = _require_hash(case, _receipt_hash(activsg_metadata, case.name), "ACTIVS MATPOWER case")
    ...
    releases = {state: inventory_root / state / f"physical-inventory-{version}.json.gz" for state in ("tx", "mn")}
    artifacts = {state: _load_release(path) for state, path in releases.items()}
    if any(artifact.get("geography_id") != state for state, artifact in artifacts.items()):
        raise MaterializationError("published inventory release geography does not match its state path")
    derived = _derived_rows(output)
    if output.exists() and not replace:
        raise MaterializationError(f"output exists; use --replace only after serializing downstream writers: {output}")
    ...
    try:
        ...
        try:
            ...
            labels = {"uri_2021": "Winter Storm Uri weather window", "beryl_2024": "Hurricane Beryl weather window"}
            for scenario_id, start, end in windows:
                con.execute("INSERT INTO scenarios VALUES (?, ?, 'historical', ?, ?, 'noaa_hrrr', ?, 'hrrr-sfc-3km', ?, ?)", [scenario_id, labels[scenario_id], start, end, str(hrrr_receipt), hrrr_metadata["retrieved_at"], f"runtime-{scenario_id}"])
            for artifact in artifacts.values():
                write_artifact(con, artifact)
            counts = _validate_output(con, sum(len(artifact["assets"]) for artifact in artifacts.values()))
        finally:
            con.close()
        receipt = {
            "receipt_kind": "flux_runtime_store", "receipt_version": 1,
            "output": str(output), "output_sha256": sha256_file(stage_db),
            "inputs": {"hrrr_db": {"path": str(hrrr_db), "sha256": hrrr_hash}, "activsg_aux": {"path": str(aux), "sha256": aux_hash}, "activsg_case": {"path": str(case), "sha256": case_hash}, "physical_releases": {state: {"path": str(path), "compressed_sha256": sha256_file(path), "content_sha256": artifacts[state]["content_sha256"]} for state, path in releases.items()}},
            "scenario_windows": [{"scenario_id": sid, "ts_start": start.isoformat(), "ts_end": end.isoformat()} for sid, start, end in windows],
            "counts": counts,
            "limitations": ["ACTIVSg2000 is synthetic Texas topology; no physical-inventory asset is joined to it.", "Minnesota physical inventory is source-backed map inventory only; no Minnesota topology or cascade claim is created.", "Weather is persisted only for its observed Uri and Beryl windows; no prediction or cascade result is created."],
        }
