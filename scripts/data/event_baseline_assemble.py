...
def catalog_rows(events_dir: Path) -> list[dict[str, object]]:
    ...
    for path in paths:
        ...
        for record in bundle["records"]:
            identity = (record["county_fips"], record["scenario_id"], record["window_start_utc"])
            if identity in identities:
                raise ValidationError(f"duplicate canonical county-window identity across bundles: {identity}")
            ...
            rows.append({
                ...
                "outage_rate": label["outage_rate"], "positive": label["positive"],
                "provenance_receipt_ids": json_list(record["provenance_receipt_ids"]),
                "source_evidence_status": record["source_evidence_status"], "source_row_keys": json_list(record["source_row_keys"]), "source_slices": json_list(record["source_slices"]),
                "uncertainties": json_list(event["uncertainties"]), "exclusions": json_list(event.get("exclusions", [])),
            })
    return sorted(rows, key=lambda row: (str(row["event_id"]), str(row["county_fips"]), str(row["scenario_id"]), str(row["window_start_utc"])))
