...
META_PATH = ROOT / "data/3d/assets/solar_array.meta.json"
TRANSFORM = {
    "length_unit": "meter",
    "unit_scale": 1.0,
    "up_axis": "Y",
...
def validate(meta: dict[str, Any], catalog: dict[str, Any]) -> list[str]:
    entry = next((x for x in catalog["archetypes"] if x["id"] == "solar_array"), None)
    if not entry:
        return ["solar archetype is missing from the shared catalog"]
    errors = []
    for field in (
        "semantic_name",
        "category",
...
def main() -> int:
    ...
    if errors:
        print("\n".join(errors))
        return 1
    print("solar_array metadata matches flux:3d-asset-archetypes:v1")
    return 0
