"""Validate the coal/retiring-site export metadata against the shared 3D catalog."""
...
META_PATH = ROOT / "data/3d/assets/coal_plant_retiring_site.meta.json"
...
def validate(meta: dict[str, Any], catalog: dict[str, Any]) -> list[str]:
    ...
    entry = next(
        (
            ...
            if item["id"] == "coal_plant_retiring_site"
        ),
        None,
    )
    if not entry:
        return ["coal archetype is missing from the shared catalog"]
    for field in (
        "archetype_id",
        "semantic_name",
        "category",
        ...
        expected = entry["id"] if field == "archetype_id" else entry[field]
...
def main() -> int:
    ...
    if errors:
        print("\n".join(errors))
        return 1
    print("coal_retiring_site metadata matches flux:3d-asset-archetypes:v1")
    return 0
