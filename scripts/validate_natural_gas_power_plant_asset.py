"""Validate natural-gas plant export metadata against the shared 3D catalog."""
...
META_PATH = ROOT / "data/3d/assets/natural_gas_power_plant.meta.json"
TRANSFORM = {
    "length_unit": "meter",
    "unit_scale": 1.0,
    "up_axis": "Y",
...
def validate(meta: dict[str, Any], catalog: dict[str, Any]) -> list[str]:
    entry = next(
        (item for item in catalog["archetypes"] if item["id"] == "natural_gas_plant"),
        None,
    )
    if not entry:
        return ["natural-gas archetype is missing from the shared catalog"]
    errors: list[str] = []
    for field in (
        "semantic_name",
        "category",
...
def main() -> int:
    ...
    if errors:
        print("\n".join(errors))
        return 1
    print("natural_gas_power_plant metadata matches flux:3d-asset-archetypes:v1")
    return 0
