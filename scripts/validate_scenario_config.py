...
def object_keys(value: object, label: str, required: set[str]) -> dict:
    if not isinstance(value, dict):
        raise ValueError(f"{label} must be an object")  # noqa: TRY004 -- malformed user configuration is a validation error
...
def validate_provenance(value: object, label: str) -> None:
    provenance = object_keys(
        value, label, {"source_id", "url", "retrieved_at_utc", "scope"}
    )
    if not all(
...
def validate_reference(
    ...
    *,
    ...
    if reference["status"] == "supported" and (
    ...
    if time_series:
        ...
        for index, sample in enumerate(reference["samples"]):
            sample = object_keys(
                sample, f"{label}.samples[{index}]", {"ts_utc", "value"}
            )
            ...
            if start is not None and end is not None and not start <= timestamp < end:
                raise ValueError(
                    f"{label}.samples[{index}] falls outside the scenario window"
                )
            if not is_finite_number(sample["value"]):
                raise ValueError(
                    f"{label}.samples[{index}].value must be a finite number"
                )
...
def validate_quantity(value: object, label: str, unit: str) -> None:
    ...
    if (
        unit == "fraction"
        and quantity["status"] == "supported"
        and quantity["value"] > 1
    ):
...
def validate(config: dict) -> None:
    ...
    scenario = object_keys(
        config["scenario"],
        "scenario",
        {"id", "label", "class", "execution_status", "time_window"},
    )
    if not isinstance(scenario["id"], str) or not re.fullmatch(
        r"[a-z][a-z0-9_]*", scenario["id"]
    ):
        raise ValueError(
    ...
    if scenario["class"] not in {
        "historical_weather_stress",
        "historical_operating_condition",
        "synthetic_stress",
        "forecast",
    }:
        raise ValueError("scenario.class is invalid")
    if scenario["execution_status"] not in {
        "example_only",
        "ready_for_adapter",
        "unavailable",
    }:
    ...
    window = object_keys(
        ...
        "scenario.time_window",
        {"start_utc", "end_utc", "interval_minutes"},
    )
    ...
    if (
        isinstance(window.get("interval_minutes"), bool)
        or not isinstance(window.get("interval_minutes"), int)
        or window["interval_minutes"] < 1
    ):
        raise ValueError(
            "scenario.time_window.interval_minutes must be a positive integer"
        )

    static_context = object_keys(
        config["static_context"], "static_context", {"geography", "network"}
    )
    references = []
    for name, reference in static_context.items():
        validate_reference(reference, f"static_context.{name}", time_series=False)
        references.append(reference)
    time_series = object_keys(
        config["time_series"],
        "time_series",
        {"demand", "weather", "outage", "availability"},
    )
    for name, reference in time_series.items():
        validate_reference(
            reference, f"time_series.{name}", time_series=True, start=start, end=end
        )
        references.append(reference)
    ...
    for index, generator in enumerate(resources["generation"]):
        generator = object_keys(
            generator,
            f"resources.generation[{index}]",
            {"resource_id", "fuel", "ramp_mw_per_min"},
        )
        if not all(
            isinstance(generator[field], str) and generator[field]
            for field in ("resource_id", "fuel")
        ):
            raise ValueError(
                f"resources.generation[{index}] resource_id and fuel must be non-empty strings"
            )
        validate_quantity(
            generator["ramp_mw_per_min"],
            f"resources.generation[{index}].ramp_mw_per_min",
            "MW/min",
        )
    ...
    for index, storage in enumerate(resources["storage"]):
        storage = object_keys(
            storage,
            f"resources.storage[{index}]",
            {"resource_id", "power_mw", "energy_mwh", "state_of_charge"},
        )
        if not isinstance(storage["resource_id"], str) or not storage["resource_id"]:
            raise ValueError(
                f"resources.storage[{index}].resource_id must be a non-empty string"
            )
        validate_quantity(
            storage["power_mw"], f"resources.storage[{index}].power_mw", "MW"
        )
        validate_quantity(
            storage["energy_mwh"], f"resources.storage[{index}].energy_mwh", "MWh"
        )
        validate_quantity(
            storage["state_of_charge"],
            f"resources.storage[{index}].state_of_charge",
            "fraction",
        )
        quantities.extend(
            (storage["power_mw"], storage["energy_mwh"], storage["state_of_charge"])
        )
    if scenario["execution_status"] == "ready_for_adapter" and (
        ...
        raise ValueError(
            "ready_for_adapter cannot activate unavailable inputs or unsupported resource capabilities"
        )
    validate_provenance(config["provenance"], "provenance")
    uncertainty = object_keys(config["uncertainty"], "uncertainty", {"level", "notes"})
    if (
        uncertainty["level"] not in {"low", "medium", "high"}
        or not isinstance(uncertainty["notes"], list)
        or not uncertainty["notes"]
        or not all(isinstance(note, str) and note for note in uncertainty["notes"])
    ):
