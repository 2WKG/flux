"""Validate battery-storage export metadata against the shared 3D catalog."""
...
META_PATH = ROOT / "data/3d/assets/battery_storage.meta.json"
TRANSFORM = {
    "length_unit": "meter",
    "unit_scale": 1.0,
    "up_axis": "Y",
...
def validate(meta: dict[str, Any], catalog: dict[str, Any]) -> list[str]:
    entry = next(
        (x for x in catalog["archetypes"] if x["id"] == "battery_storage"), None
    )
    if not entry:
        return ["battery archetype is missing from the shared catalog"]
    errors = []
    for field in (
        "semantic_name",
        "category",
...
def main() -> int:
    ...
    if errors:
        print("\n".join(errors))
        return 1
    print("battery_storage metadata matches flux:3d-asset-archetypes:v1")
    return 0
