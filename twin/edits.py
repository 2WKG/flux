...
def add_generator(
    element_id: str, bus_id: int, p_mw: float, *, pmax_mw: float | None = None
) -> GridEdit:
    return GridEdit(
        "add_gen",
        _id(element_id),
        bus_id=int(bus_id),
        p_mw=_positive(p_mw, "p_mw"),
        pmax_mw=_positive(pmax_mw if pmax_mw is not None else p_mw, "pmax_mw"),
    )


def add_load(element_id: str, bus_id: int, p_mw: float) -> GridEdit:
    return GridEdit(
        "add_load", _id(element_id), bus_id=int(bus_id), p_mw=_positive(p_mw, "p_mw")
    )
...
def add_line(
    ...
    *,
    ...
    return GridEdit(
        "add_line",
        _id(element_id),
        from_bus_id=int(from_bus_id),
        to_bus_id=int(to_bus_id),
        r_pu=_nonnegative(r_pu, "r_pu"),
        x_pu=_positive(x_pu, "x_pu"),
        rate_a_mw=_positive(rate_a_mw, "rate_a_mw"),
        base_kv=_positive(base_kv, "base_kv"),
        length_km=_positive(length_km, "length_km"),
    )
...
def edit_hash(edits: Iterable[GridEdit]) -> str:
    ...
    """
    payload = [
        edit.json() if isinstance(edit, GridEdit) else _bad_edit(edit) for edit in edits
    ]
...
def _apply(net: Any, edit: GridEdit) -> None:
    ...
    if edit.kind in {"outage", "remove"}:
        try:
            ...
            raise SimulationInputError(
                f"unknown synthetic element {edit.element_id!r}"
            ) from exc
        if table == "ext_grid":
            raise SimulationInputError(
                "removing the grid-forming reference requires an explicit replacement model"
            )
        net[table].at[index, "in_service"] = False
        return
    if edit.element_id in lookup:
        raise SimulationInputError(
            f"duplicate synthetic element id {edit.element_id!r}"
        )
    if edit.kind == "add_gen":
        ...
        index = pp.create_gen(
            net,
            bus,
            p_mw=float(edit.p_mw),
            vm_pu=1.0,
            max_p_mw=float(edit.pmax_mw),
            min_p_mw=0.0,
            name=edit.element_id,
        )
        table = "gen"
    elif edit.kind == "add_load":
        bus = _bus(net, edit.bus_id)
        index = pp.create_load(
            net, bus, p_mw=float(edit.p_mw), q_mvar=0.0, name=edit.element_id
        )
        table = "load"
        ...
        index = pp.create_line_from_parameters(
            ...
            first,
            second,
            float(edit.length_km),
            float(edit.r_pu) * z_base / float(edit.length_km),
            float(edit.x_pu) * z_base / float(edit.length_km),
            0.0,
            float(edit.rate_a_mw) / (3**0.5 * float(edit.base_kv)),
            name=edit.element_id,
        )
        table = "line"
...
def _bad_edit(value: object) -> None:
    raise SimulationInputError(
        f"edits must contain GridEdit values, got {type(value).__name__}"
    )
