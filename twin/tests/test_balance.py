...
def _net():
    ...
    buses = [pp.create_bus(net, 230.0, name=f"bus-{source_id}") for source_id in (101, 102, 103)]
    net["flux_bus_index"] = {101: buses[0], 102: buses[1], 103: buses[2]}
    net["flux_bus_metadata"] = {
        buses[0]: {"bus_id": 101, "state": "TX", "ba_code": "ERCO", "county_fips": "48001"},
        buses[1]: {"bus_id": 102, "state": "TX", "ba_code": "ERCO", "county_fips": "48003"},
        buses[2]: {"bus_id": 103, "state": "TX", "ba_code": "ERCO", "county_fips": "48005"},
    }
    net["flux_element_lookup"] = {}

    line_101_102 = pp.create_line_from_parameters(net, buses[0], buses[1], 1.0, 0.01, 0.1, 0.0, 1.0, name="line:10")
    line_102_103 = pp.create_line_from_parameters(net, buses[1], buses[2], 1.0, 0.01, 0.1, 0.0, 1.0, name="line:11")
    ...
    coal = pp.create_gen(net, buses[0], p_mw=40.0, vm_pu=1.0, max_p_mw=100.0, name="generator:2")
    wind = pp.create_gen(net, buses[1], p_mw=10.0, vm_pu=1.0, max_p_mw=50.0, name="generator:3")
...
def test_state_ba_and_county_scopes_use_only_declared_bus_identity(scope, scope_id, expected_draw, expected_capability):
...
def test_edited_island_scope_uses_same_in_service_connectivity_as_cascade():
    net = _net()
    result = balance_report(net, scope="island", scope_id=101, edits=[outage("line:10")])
    ...
    assert result["edit_hash"] != balance_report(net, scope="island", scope_id=101)["edit_hash"]
