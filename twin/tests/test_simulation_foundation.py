...
def _fixture_db(tmp_path):
    ...
    with duckdb.connect(str(path)) as con:
        ...
        con.execute(
            "INSERT INTO buses VALUES (10, 'slack', 110, -97, 30, '48001'), (20, 'load', 110, -97.1, 30.1, '48003'), (30, 'island', 220, -97.2, 30.2, '48003')"
        )
        con.execute(
            "INSERT INTO lines VALUES (1, 10, 20, 110, 0.01, 0.1, 100, 2, false), (2, 20, 30, 220, 0.01, 0.1, 30, 1, true)"
        )
        ...
        con.execute(
            "INSERT INTO synthetic_generator_electrical VALUES (1, 10, 0, 10, -10, 0, 1), (2, 20, 0, 20, -20, 0, 1)"
        )
        con.execute("INSERT INTO loads VALUES (1, 20, 10), (2, 30, 20)")
        con.execute(
            "INSERT INTO critical_loads VALUES (7, 'hospital', 'fixture hospital', 30)"
        )
    return path
...
def test_edits_are_immutable_order_sensitive_and_validate_identity(tmp_path):
    ...
    edits = (
        outage("line:1"),
        add_generator("generator:new", 20, 5),
        add_load("load:new", 20, 2),
    )
    ...
    changed = apply_edits(
        net,
        (add_line("line:new", 10, 30, r_pu=0.01, x_pu=0.1, rate_a_mw=50, base_kv=110),),
    )
...
def test_cascade_sheds_island_and_attributes_modeled_load_without_customers(tmp_path):
    ...
    assert {event["element_id"] for event in result["tripped_element_ids"]} >= {
        "impedance:2",
        "load:2",
    }
    assert result["critical_loads_lost"] == [
        {"cl_id": "7", "kind": "hospital", "name": "fixture hospital"}
    ]
    assert result["county_impacts"] == [
        {
            "county_fips": "48003",
            "lost_mw": 20.0,
            "fraction_dark": 0.666667,
            "basis": "synthetic modeled load; customer count unavailable",
        }
    ]
    assert (
        island_primitives(net, (outage("impedance:2"),))[1]["has_grid_forming_source"]
        is False
    )
