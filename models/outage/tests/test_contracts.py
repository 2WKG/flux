...
def test_fixture_cannot_masquerade_as_an_observed_label():
    fixture = FixtureLabel(
        customers_out_max=10, total_customers=100, reason="pre-ingest"
    )
...
def test_observed_label_requires_provenance():
    with pytest.raises(ValidationError):
        ObservedLabel(
            customers_out_max=10, total_customers=100
        )  # no hash / dataset / time
...
def test_observed_label_rejects_impossible_counts():
    with pytest.raises(ValidationError, match="exceeds total_customers"):
        ObservedLabel(
            customers_out_max=101,
            total_customers=100,
            source_dataset_id="eaglei-2021",
...
def test_feature_row_reports_what_is_missing():
    row = FeatureRow(
        ...
        features=(
            (
                "gust_max",
                FeatureValue(value=22.5, status=FeatureStatus.PRESENT, unit="m_s"),
            ),
            (
                "ice_sum_48h",
                FeatureValue(status=FeatureStatus.MISSING_SOURCE, unit="mm"),
            ),
        ),
    )
    ...
    with pytest.raises(TypeError):
        row.features[0] = (
            "gust_max",
            FeatureValue(value=0, status=FeatureStatus.PRESENT, unit="m_s"),
        )
...
def test_trained_prediction_is_unevaluated_until_an_evaluation_exists():
    p = TrainedModelPrediction(
        p_out=0.4, customers_at_risk=100, driver=Driver.ICE, artifact=_artifact()
    )
...
def test_probability_is_bounded():
    with pytest.raises(ValidationError):
        TrainedModelPrediction(
            p_out=1.4, customers_at_risk=1, driver=Driver.ICE, artifact=_artifact()
        )
...
def test_row_matches_the_six_pinned_columns():
    rec = PredictionRecord(
        ...
        prediction=TrainedModelPrediction(
            p_out=0.42, customers_at_risk=1234, driver=Driver.ICE, artifact=_artifact()
        ),
    )
