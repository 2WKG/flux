...
``pipelines.db`` DDL (``SCHEMA_VERSION`` contract, with the ``scenarios``/``counties``
...
def con(tmp_path: Path):
    """A real ``SCHEMA_VERSION`` database file with the companion tables and FK parents."""
...
def test_schema_goes_through_the_shared_contract():
    ...
    ensure_persistence_schema(con)
    assert stored_contract_version(con) == SCHEMA_VERSION
