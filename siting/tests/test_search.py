...
def _adapters(*, p4_by_candidate: dict[str, bool] | None = None) -> SearchAdapters:
    ...
    def feasibility(**kwargs):
        return {"feasible": kwargs["candidate"]["candidate_id"] != "unsafe", "flags": ["S1"]}

    def balance(**kwargs):
        candidate_id = kwargs["candidate"]["candidate_id"]
        return {
            "p4_passed": (p4_by_candidate or {}).get(candidate_id, True),
            "headroom_mw": {"load-a": 80.0, "load-b": 120.0, "bad-corridor": 999.0}.get(candidate_id, 50.0),
        }
    ...
    def redundancy(**kwargs):
        ...
        if candidate is None:
            return {"score": 10.0}
        return {"score": {"small-headroom": 60.0, "large-headroom": 20.0, "load-a": 80.0, "load-b": 20.0}.get(candidate["candidate_id"], 30.0)}

    def cascade(**kwargs):
        candidate_edits = kwargs["edits"]
        hour = kwargs["hour"]
        candidate = next((edit["element_id"].rsplit(":", 1)[-1] for edit in candidate_edits if edit["kind"] in {"add_gen", "add_load"}), "base")
    ...
    return SearchAdapters(
        ...
        cascade=cascade,
        edit_factory=lambda **kwargs: {key: value for key, value in kwargs.items() if value is not None},
    )
...
def _net() -> FakeNet:
    return FakeNet(
        site_candidates=[
            {"candidate_id": "small-headroom", "bus_id": "bus-1", "safety_flags": ["S1"]},
            {"candidate_id": "large-headroom", "bus_id": "bus-2", "safety_flags": ["S2"]},
...
def test_producer_objective_is_counterfactual_and_not_naive_headroom() -> None:
    results = search_locations(
        _net(), kind="producer", unit_mw=300, scenario_id="synthetic-stress", n=2,
        adapters=_adapters(),
    )

    assert [result["candidate_id"] for result in results] == ["small-headroom", "large-headroom"]
    top = results[0]
...
def test_consumer_rejects_p4_breach_even_with_largest_headroom() -> None:
    results = search_locations(
        _net(), kind="consumer", unit_mw=100, scenario_id="synthetic-stress", n=2,
...
def test_consumer_requires_explicit_corridor_pass() -> None:
    ...
    assert search_locations(_net(), kind="consumer", unit_mw=100, scenario_id="s", adapters=adapters) == []
...
def test_full_window_work_is_bounded_to_five_candidates() -> None:
    ...
    adapters = SearchAdapters(
        ...
        redundancy=lambda **kwargs: {"score": 1.0 if kwargs["candidate"] is None else float(kwargs["candidate"]["candidate_id"].split("-")[-1])},
        cascade=lambda **kwargs: {"lost_load_mwh": 100.0 if not kwargs["edits"] else 90.0, "congestion_mwh": 100.0},
        edit_factory=adapters.edit_factory,
    )
    results = search_locations(net, kind="producer", unit_mw=100, scenario_id="s", n=5, adapters=adapters)
