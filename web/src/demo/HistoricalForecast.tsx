...
export function HistoricalForecastPanel({
  forecast,
  countyFipses = [],
  selectedCountyFips,
  onCountyChange,
}: {
  forecast: HistoricalCountForecast;
  /** Values are supplied from the forecast artifact's `scope.observed_county_fips`. */
  countyFipses?: readonly string[];
  ...
  return <section className="historical-forecast" aria-label="Historical experimental count trajectory">
    <header><p className="control-room__eyebrow">Historical experimental count trajectory</p><h3>{forecast.countyName ?? forecast.countyFips ?? "Selected county"}</h3>{countyFipses.length > 0 && onCountyChange ? <label>Reviewed county <select aria-label="Reviewed historical county" value={selectedCountyFips ?? forecast.countyFips ?? ""} onChange={(event) => onCountyChange(event.target.value)}>{countyFipses.map((countyFips) => <option key={countyFips} value={countyFips}>{countyFips}</option>)}</select></label> : null}</header>
