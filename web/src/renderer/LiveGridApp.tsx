...
function LiveGridApp() {
  ...
  return <main className="grid-app" data-runtime="spatial-api">
    ...
    <section className="grid-map" aria-label="Source-backed physical inventory map"><Map key={state} ref={mapRef} initialViewState={state === "tx" ? { longitude: -99, latitude: 31, zoom: 5 } : { longitude: -94, latitude: 46, zoom: 5.6 }} mapStyle={OFFLINE_BASEMAP_STYLE} onMoveEnd={(event) => { const b = event.target.getBounds(); setBbox([b.getWest(), b.getSouth(), b.getEast(), b.getNorth()].join(",")); }}><Source id="physical-inventory-source" type="geojson" data={featureCollection as never}><Layer id="physical-inventory-lines" type="line" paint={{ "line-color": "#3ee3ff", "line-width": 4 }} /></Source></Map><svg className="grid-geometry-overlay" viewBox="0 0 1000 1000" preserveAspectRatio="none" aria-label="Visible source geometry">{overlayGeometry.map(({ feature, points }) => feature.geometry.type.includes("Point") ? points.map(([x, y], index) => <circle key={`${feature.id}:${index}`} cx={x} cy={y} r="9" onClick={(event) => { event.stopPropagation(); setSelected(feature.properties); }} />) : <polyline key={feature.id} points={points.map((point) => point.join(",")).join(" ")} onClick={(event) => { event.stopPropagation(); setSelected(feature.properties); }} />)}</svg>
