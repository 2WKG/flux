...
def create_interactive_service(
    *, duckdb_path: Path, case_path: Path
) -> InteractiveService:
...
def _scenario_edit(
    payload: ScenarioEditRequest, *, duckdb_path: Path, case_path: Path
) -> dict[str, object]:
...
def _cascade(
    payload: CascadeRequest, *, duckdb_path: Path, case_path: Path
) -> dict[str, object]:
...
def _balance(
    ...
    *,
    ...
    return _result(
        ...
        case_path,
        balance_report(
            immutable_scenario_net(baseline, list(edit.element_ids))
            if edit
            else baseline
        ),
    )
...
def _redundancy(
    bus_id: int, *, scenario_id: str, hour: int, duckdb_path: Path, case_path: Path
) -> dict[str, object]:
...
def _siting_search(
    payload: SitingSearchRequest, *, duckdb_path: Path, case_path: Path
) -> dict[str, object]:
...
def _result(net: Any, case_path: Path, data: object) -> dict[str, object]:
    return {
        ...
        "limitations": [
            ...
            "Interactive edits stay in memory and never write DuckDB.",
        ],
        "data": data,
    }
