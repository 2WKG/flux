...
def _db(path, *, scenarios=("uri_2021",), source_kind="fixture"):
    ...
    con.execute(
        "CREATE TABLE line_upgrade_detail (line_id BIGINT, scenario_id TEXT, best_tech TEXT, congestion_method TEXT, region TEXT)"
    )
    for index, scenario in enumerate(scenarios, 1):
        con.execute("INSERT INTO lines VALUES (?, 1, 2, 230)", [index])
        con.execute(
            "INSERT INTO line_upgrade_scores VALUES (?, ?, 'v1', ?, 'fixture', 'fixture-ref', ?, 100, 20, 10, 1000000, 2000000, ?, true, false, NULL)",
            [
                index,
                scenario,
        ...
        con.execute(
            "INSERT INTO line_upgrade_detail VALUES (?, ?, 'dlr', 'exact', 'ERCOT')",
            [index, scenario],
        )
    con.close()
