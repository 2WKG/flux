...
_artifact_path = (
    ...
    / "data/artifacts/synthetic_topology/tx/activsg2000-current-v1.json.gz"
)
...
def _read_packaged_geometry() -> dict[str, object]:
    ...
    if manifest.get("compressed_sha256") != hashlib.sha256(compressed).hexdigest():
        raise ValueError(
            "packaged synthetic topology checksum does not match its manifest"
        )
    raw = gzip.decompress(compressed)
    if manifest.get("content_sha256") != hashlib.sha256(raw).hexdigest():
        raise ValueError(
            "packaged synthetic topology content checksum does not match its manifest"
        )
    artifact = json.loads(raw)
    payload = artifact.get("payload")
    if (
        not isinstance(payload, dict)
        or artifact.get("artifact_id") != "tx:synthetic-topology:activsg2000-current-v1"
    ):
...
def _select_elements(
    payload: dict[str, object], element_ids: list[str] | None
) -> dict[str, object]:
    data = payload["data"]
    ...
    requested = (
        sorted(by_id) if element_ids is None else [str(value) for value in element_ids]
    )
    elements = [
        by_id.get(
            value,
    ...
    if unresolved:
        result["reason"] = (
            "one or more requested synthetic elements could not be resolved"
        )
    return result
