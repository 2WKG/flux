...
def _read_model_geometry(path: Path, element_ids: list[str] | None) -> dict[str, object]:
    ...
    requested = sorted(all_elements) if element_ids is None else [str(value) for value in element_ids]
    elements = [all_elements.get(value, {"element_id": value, "resolved": False, "reason": "unknown synthetic model element"}) for value in requested]
