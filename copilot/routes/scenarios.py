...
def _scenario_from_row(row: tuple[object, ...]) -> ScenarioRow:
    ...
    if source_version is not None and not isinstance(source_version, str):
        raise _RowInvalid("source_version must be a string when present")
    retrieved_at = _as_utc(source_retrieved_at)
    if source_retrieved_at is not None and retrieved_at is None:
        raise _RowInvalid("source_retrieved_at must be a timestamp when present")
